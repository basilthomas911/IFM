...
| 9 | **Implemented; qualification in progress** | PostgreSQL 17 full plus six physical incrementals verify/combine/boot; a live AWS signed full-plus-six chain stages from both vaults; intentional WAL gap, fill, replay, continuity, and recovery replication pass. | Complete simultaneous activity, restart, slow-S3/source-failover, spool pressure, and replica-lag alarm drills. |
| 10 | **Implemented; qualification in progress** | Exact seven-point dependency staging passes from both vaults; physical chain boot passes; a real PostgreSQL 17 timestamp PITR boot passes after fixing the server-rejected recovery timestamp format. | Complete AWS-to-native fresh-target drills from both vaults, the negative matrix, application read/write validation, and component RPO/RTO measurement. |
...
| 14 | **Not started** | Dependency-aware retention, legal-hold, exact-version plan, and constrained-deletion requirements are documented. | Implement dry-run/approval/execution/reconciliation and pass drift, hold, compliance, dependency, replica, and partial-failure tests. |
| 15 | **Not started** | Source-neutral UI/Console contracts exist; required AWS metrics, health behavior, and runbooks are enumerated. | Add projected AWS status, dashboards, alarms, runbooks, health isolation, bounded retries, and exception-noise qualification. |
| 16 | **Not started** | Security, resilience, performance, capacity, and cost test scope is documented. | Execute scans, fault injection, workload/restore tests, cost measurement, remediation, and formal risk acceptance. |
