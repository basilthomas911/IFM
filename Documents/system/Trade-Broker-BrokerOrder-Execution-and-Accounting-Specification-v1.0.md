...
### 7.1 IFM Execution Order Algorithms — directed atomic combinations

This subsection records the target behavior for multi-leg futures-option orders; it is **not an implemented capability**. The UI design is [Broker Order/Fills](ES-Trade-Blotter-Three-Tab-Design/Tab-2a-Broker-Order-and-Order-Fills.md). The selected **Execution algorithm** defaults to `None`. `None` submits one ordinary broker order using the separately selected `Limit` or `Market` order type, subject to the approved order policy. The V1.2 execution-policy addendum still makes Limit the normal default and reserves Market for explicitly authorized defensive escalation; selecting `None` does not waive that guard.

For a multi-leg strategy, the additional algorithm is **IFM Atomic Combo Microexecution**. It is IFM's own policy, not IBKR Adaptive. A complete vertical or iron condor is submitted as **one directed exchange `BAG` combination order** with the exact approved legs, ratios and net price. It must not use `SMART`, split the legs into independent broker orders, or silently fall back to either. Before dispatch, the adapter must qualify the specific ES futures-option contract set, destination exchange, account permission, supported leg count, order type, tick/sign convention and atomic-execution semantics. If any of these are unverified or rejected, fail locally and show the reason; do not route an alternative.

An exchange-routed combination may fill fewer than the requested **whole combination units**, but a unit must contain all approved legs in ratio. Broker leg execution reports must reconcile to balanced whole units. Any apparently unbalanced or incomplete evidence blocks further mutation and enters reconciliation/escalation; it is never labelled an atomic fill merely because the parent status says Filled. IBKR documents direct-routed combinations as a single transaction and SmartRouted combinations as potentially leg-separate: [IBKR combo order types](https://interactivebrokers.github.io/tws-api/basic_orders.html). This general behavior does not by itself prove that every four-leg ES futures-option BAG is accepted by a chosen venue; paper/venue qualification remains mandatory.

| UI algorithm choice | Broker order type | IFM behavior |
| --- | --- | --- |
| `None` (default) | `Limit` (default) or policy-permitted `Market` | Submit one qualified directed combination order; no automated price chasing. |
| `IFM Atomic Combo Microexecution` | `Limit` | Submit one qualified directed combo Limit order and work its **net** limit under the frozen approved envelope. No per-leg repricing or automatic Market conversion. |

The IFM algorithm offers an **Execution pace** of `Patient`, `Normal` (default), or `Urgent`. These names express IFM cadence and bounded net-price steps, not IBKR's `adaptivePriority`. Profile parameters are versioned and separately approved for opening and closing: initial net limit rule, quote freshness, minimum time between amendments, ticks per amendment, maximum amendments, total deadline, maximum slippage/minimum acceptable credit or maximum debit, no-fill action, and permitted cancel/reconcile behavior. Patient waits longer and uses smaller steps; Urgent may act sooner within the same hard envelope. No profile may guarantee a fill or exceed its approved price/risk bounds.

**First example — opening one ES short iron condor.** Market Selection chooses four exact futures-option contracts for one expiry: buy the lower put wing `+LP`, sell the put `-SP`, sell the call `-SC`, and buy the upper call wing `+LC`. Portfolio and Risk Manager approve one four-leg, one-unit opening component, an account and directed venue, a signed net-credit convention, a minimum acceptable credit, a deadline and one frozen Normal profile. The BrokerOrder actor durably places one four-leg `BAG` Limit order at an initial tick-correct net credit within the envelope. It waits for an exact broker acknowledgement before any update. If a fresh, materially changed quote and the profile's cadence permit, it computes the next tick-correct **net** limit; only one revision-bound update may be outstanding. A fill of that one combo unit completes only after all four leg executions reconcile. On a rejection, stale quote, unknown dispatch outcome, lost callback or deadline, it stops normal repricing and follows the authorized cancel/reconcile/escalate path. The example does not prescribe live prices or timings; those belong to a qualified profile and approved order.

The UI must separately show `Order type`, `Time in force`, `Execution algorithm`, and conditional `Execution pace`; the pace is hidden/disabled for `None`. It must display directed venue and **Atomic combo / No SMART** as read-only qualified facts, current net limit, approved worst net limit, midpoint/quote age, profile/version, amendment count and next decision/deadline. Submission requires an explicit preview of four legs, ratios, signed net credit/debit, account, route, limit, TIF and approval status. After submission, show broker order ID, acknowledged revision, working/partial whole-combo quantity, last action and reason, and any reconciliation lock. Market and GTC availability must be determined by the order policy and venue qualification, not inferred from the UI choice alone.

This is an actor-owned order policy: the UI selects and displays it but does not run timers, call IBKR, or reprice locally. The existing durable intent, acknowledgement, revision, source-epoch, idempotency and reconciliation requirements in this specification apply to every step.
...
The desktop trade UI exposes three first-release strategy shapes: one-leg Futures, two-leg Vertical Spread, and four-leg Iron Condor. Each has a trade-order editor, a trade blotter, and a trade/position detail view. Strategy-specific controls show their exact legs, side, quantity, limit and fill allocation, while a shared order-evidence model shows Portfolio approval, TradeOrder, OrderExecution, BrokerOrder, account gate, operation receipts, acknowledgement, partial/complete fills, commissions, Fund accounting and classified failures. Opening manual orders use the same Portfolio-approved broker channel as strategy orders; Closing orders show their originating position and opposite execution. Historical or unapproved data is read-only, and a local dispatch receipt never appears as a fill or completed trade. Navigation and queries use the full Portfolio/Fund/Order/Trade identity, exact strategy kind and date range. The existing Iron Condor editor/blotter is migrated to this common evidence contract; Futures and Vertical Spread controls are added beside it.
...
7. **UI/API and operations:** Update manual trade entry to Portfolio-approved broker channel; add Futures and Vertical Spread trade-order editors, blotters and trade/position detail views alongside the migrated Iron Condor views; show shared execution and Fund accounting evidence in each; add three-environment cash-backed account management, qualification/approval/cap/gate/discrepancy views and Actor Health metrics; audit permissions and redaction.
