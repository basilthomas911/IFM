...
As of 2026-08-20, UI gates G0 through G4 have accepted Development results. G1 proves the initialized shell and status history, current market-outlook state, ES and VX sidebar charts, all five economic-calendar ranges, supported Market Data and Reference catalogs, named-fund and existing trade views, supported System Administration behavior, modal reopen, normal close, and complete process/listener cleanup. G2 proves the non-production mutation policy, dependency probes, a healthy 90-actor backend, reversible typed baseline capture, real WinForms initialization, exact command/event correlation, feed control, securities and reference maintenance, production FMP imports, lookup maintenance, reusable-fund compensation, order/trade workflows, supported EOD, allowlisted backup, exact baseline restoration, and bounded normal shutdown through G2-038. G3 proves the complete UI event-consumer catalog, MessagePack command-response delivery over live NATS, lossless FIFO streams, latest-value and per-key coalescing, bounded histories, timeframe identity, diagnostics, stop rejection, and exactly-one listener after reopen. G4 proves bounded startup failure, distinct broker/backend diagnostics, disconnect/reconnect correlation, three repeated initialized lifecycle cycles, a responsive bounded 10,000-event burst, close during a severed in-flight request, deterministic modal evidence, and complete process/network cleanup. Operator acceptance remains open, so Milestone A is not yet complete. Legacy scheduled-task UI remains deferred until that workflow is redesigned; it is not advertised as a supported G1 System Administration destination.
