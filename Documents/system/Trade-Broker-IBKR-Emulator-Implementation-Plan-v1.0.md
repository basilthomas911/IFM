...
TomasAI.IFM.UI.Net.Views/Trade/
  Futures/{FuturesTradeOrderView,FuturesView}.cs
  VerticalSpread/{VerticalSpreadTradeOrderView,VerticalSpreadView}.cs
  IronCondor/{IronCondorTradeOrderView,IronCondorView}.cs (migrate evidence binding)
  {TradeOrderEditorForm,TradeBlotterFactory}.cs (route all three shapes)
TomasAI.IFM.UI.Net.ViewModels/Trade/
  Futures/{FuturesTradeOrderViewModel,FuturesViewModel}.cs
  VerticalSpread/{VerticalSpreadTradeOrderViewModel,VerticalSpreadViewModel}.cs
  IronCondor/*.cs (migrate evidence binding)
  BrokerOrderEvidence/*.cs (shared order/fill/fee/accounting state)
TomasAI.IFM.UI.Net.Services/Trade/
  {BrokerOrderEvidenceQueryService,BrokerAccountQueryService}.cs
```
...
E9 delivers complete strategy-specific trade UI for all three E1 order shapes. Add a one-leg Futures trade-order editor, blotter and trade/position detail view, and a two-leg Vertical Spread editor, blotter and detail view. Migrate the existing four-leg Iron Condor editor and blotter to the same BrokerOrder evidence model. A shared query/view-model boundary presents Portfolio approval, TradeOrder, OrderExecution, BrokerOrder operations, acknowledgement, exact per-leg fills, commissions, account gate, Fund accounting and classified failure, while each strategy control renders its own leg geometry, side, quantities, approved limit and position history. Opening manual orders enter Portfolio approval; Closing orders display the originating position and opposite execution. The editor permits price-only update or individual cancel only when the durable order state, approved envelope and account gate allow the action. A local dispatch receipt, partial fill, pending commission or pending Fund accounting is shown as its actual status, not a completed trade. Historical and unsupported shapes remain read-only with a classified reason. Update `TradeOrderEditorForm`, `TradeBlotterFactory` and app tab navigation so selecting any supported Futures, Vertical Spread or Iron Condor trade opens the matching control by exact strategy kind and full Portfolio/Fund/Order/Trade ID rather than a symbol match. The broker/account evidence panel and dark trading theme are common across these controls.
...
| **E9 API/UI/health** | Single active synthetic account; Futures, Vertical Spread and migrated Iron Condor trade-order editors, blotters and trade/position detail views; shared BrokerOrder/account/Fund evidence and Actor Health; manual UI converges on Portfolio approval. | API/UI build, three-shape editor/blotter navigation, Opening/Closing and read-only history, exact leg/fill/status rendering, source/fund/date-range queries, redaction/gate/status acceptance. |
