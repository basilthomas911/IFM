...
| `TomasAI.IFM.Domain.Strategy.Contracts.Shared/Portfolio/ViewModels/FundCompositionProjectionReadModels.cs` | Portfolio/Fund/Order identity, `AggregateVersion`, workflow revision, selected template/profile and result references | Planned-order identity and composition state, not live broker order/position authority. Trade instructions do not contain resolved option-contract legs |
