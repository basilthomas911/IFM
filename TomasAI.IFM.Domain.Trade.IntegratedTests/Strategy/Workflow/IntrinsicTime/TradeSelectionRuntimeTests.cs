    ...
    public async Task Real_Postgres_assignment_paging_crosses_history_and_returns_seventeen_row_overflow_sentinel()
    {
        ...
        try
        {
            for (var n = 0; n < 86; n++)
            {
                ...
                await db.UpsertAssignmentAsync(PortfolioProjection<FundTradeTemplateAssignmentReadModel>.Create(row, n + 1, 3000000 + n, now));
            }
            ...
            await db.UpsertAssignmentAsync(PortfolioProjection<FundTradeTemplateAssignmentReadModel>.Create(extra, 87, 3000087, now));
