using MessagePack;
using TomasAI.IFM.Shared.EventModelActor.Contracts;
using TomasAI.IFM.Shared.EventModelActor;
using TomasAI.IFM.Shared.EventSourcing;
using TomasAI.IFM.Shared.Trade.ViewModels;
using TomasAI.IFM.Shared.Trade;

namespace TomasAI.IFM.Shared.Trade.Events
{

    [MessagePackObject(AllowPrivate = true)]
    public record TradePositionChangedEvent : IEvent
    {
        [IgnoreMember] public const string Actor = "TradePositionEvent";
        [IgnoreMember] public const string Verb = "Changed";
        [IgnoreMember] public const int ErrorCode = 0;
        [IgnoreMember] static readonly string CachedUserName = $"{Environment.UserDomainName}\\{Environment.UserName}";

        [Key(0)] public ActorSubject Subject { get; init; }
        [Key(1)] public Guid Id { get; init; }
        [Key(2)] public string EntityId { get; init; }
        [Key(3)] public long EventId { get; init; }
        [Key(4)] public Guid CommandId { get; init; }
        [Key(5)] public string AggregateId { get; init; }
        [Key(6)] public string EventSource { get; init; }
        [Key(7)] public DateTime ReceivedOn { get; init; }

        // payload (keys 8..)
        [Key(8)] public TradePositionEntityId TradePositionId { get; init; }
        [Key(9)] public TradePositionChangeSourceType TradePositionChangeSource { get; init; }
        [Key(10)] public string OptionLegId { get; init; }
        [Key(11)] public TradePositionReadModel PutTradePosition { get; init; }
        [Key(12)] public TradePositionReadModel CallTradePosition { get; init; }
        [Key(13)] public DateTime UpdatedOn { get; init; }
        [Key(14)] public string UpdatedBy { get; init; }

        [IgnoreMember] public string UserName => CachedUserName;
        [IgnoreMember] public string EventName => nameof(TradePositionChangedEvent);
        [IgnoreMember] public EventType EventType => EventType.DomainEvent;

        public TradePositionChangedEvent() { }

        [SerializationConstructor]
        public TradePositionChangedEvent(
            ActorSubject subject,
            Guid id,
            string entityId,
            long eventId,
            Guid commandId,
            string aggregateId,
            string eventSource,
            DateTime receivedOn,
            TradePositionEntityId tradePositionId,
            TradePositionChangeSourceType tradePositionChangeSource,
            string optionLegId,
            TradePositionReadModel putTradePosition,
            TradePositionReadModel callTradePosition,
            DateTime updatedOn,
            string updatedBy)
        {
            Subject = subject;
            Id = id;
            EntityId = entityId;
            EventId = eventId;
            CommandId = commandId;
            AggregateId = aggregateId ?? string.Empty;
            EventSource = eventSource ?? string.Empty;
            ReceivedOn = receivedOn;
            TradePositionId = tradePositionId;
            TradePositionChangeSource = tradePositionChangeSource;
            OptionLegId = optionLegId ?? string.Empty;
            PutTradePosition = putTradePosition;
            CallTradePosition = callTradePosition;
            UpdatedOn = updatedOn;
            UpdatedBy = updatedBy ?? string.Empty;
        }
    }

}
