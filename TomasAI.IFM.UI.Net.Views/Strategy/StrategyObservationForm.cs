...
public sealed class StrategyObservationForm:DarkTradingForm,IForm<StrategyObservationForm>
{
    ...
    readonly StrategyWorkflowDetailsAccordion _details=new();
    ...
    public StrategyObservationForm(IMarketConditionAssessmentQueryApi assessments,IIntrinsicTimeStrategyWorkflowQueryApi workflows,IRiskQueryApi? riskApi=null)
    {
        ...
        foreach(var input in new Control[]{_profile,_horizon,_workflow,_history}) {input.BackColor=Color.Black;input.ForeColor=Color.White;}
    ...
    async Task RunAsync(Func<int,Task> operation)
    {
        ...
        catch(Exception ex){if(!IsDisposed&&revision==_revision)_details.ShowMessage(ex.Message);}
    ...
    async Task LoadHistoryAsync(int revision)
    {
        ...
        _details.ShowMessage(_history.Items.Count==0?"No assessment history for this profile and timeframe.":"Select a workflow to inspect its assessment.");
    }
    async Task OpenWorkflowAsync(int revision)
    {
        ...
        {
            _view=view;
            _risk.Enabled=_riskApi is not null && view.CurrentStage==StrategyWorkflowStage.RiskManagement && view.OrderComposition is { Result: { CompositionResult: { Candidate: not null } } };
            var comparison=MarketAssessmentPresenter.Render(view,projected,DateTime.UtcNow);
            _details.Bind(StrategyWorkflowPresentation.CreateDetails(view,comparison));
        }
    }
