...
public sealed class PortfolioEditorForm : DarkTradingForm
{
    ...
    public PortfolioEditorForm(int portfolioId, PortfolioReadModel? source = null)
    {
        ...
        var body = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 2, RowCount = 7, Padding = new Padding(12), BackColor = PortfolioUiStyle.Surface };
        body.ColumnStyles.Add(new(SizeType.Absolute, 220)); body.ColumnStyles.Add(new(SizeType.Percent, 100));
        Add(body, 0, "Portfolio ID", _id); Add(body, 1, "Name", _name);
        Add(body, 2, "Base Currency", _currency); Add(body, 3, "Operating State", _state);
        Add(body, 4, "Broker Accounts (CSV)", _brokerAccounts); Add(body, 5, "Effective From", _effective);
        body.Controls.Add(_error, 0, 6); body.SetColumnSpan(_error, 2);
