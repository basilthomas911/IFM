...
/// <summary>
/// Provides extension methods for mapping API query endpoints to an application's routing configuration.
/// </summary>
/// <remarks>The QueryMaps class contains static methods that extend the routing builder interface, enabling
/// registration of various query endpoints related to funds, transactions, market data, reference data, options,
/// trades, and system administration. Use these methods to configure query routes in web applications that utilize
/// endpoint routing.</remarks>
static public class QueryMaps
{
    /// <summary>
    /// Configures API query endpoints for the application by mapping a set of query routes to the specified endpoint
    /// route builder.
    /// </summary>
    /// <remarks>This method maps multiple query endpoints related to funds, transactions, market data,
    /// reference data, options, trades, and system administration. It should be called during application startup to
    /// ensure all query routes are available. The provided logger can be used to monitor or troubleshoot the mapping
    /// process.</remarks>
    /// <param name="endpoints">The endpoint route builder used to define and register API query routes.</param>
    /// <param name="logger">The logger instance used for logging information and diagnostics during endpoint mapping.</param>
    public static void MapApiQueries(this IEndpointRouteBuilder endpoints, ILogger logger)
    {
        endpoints
            .MapFundQueries()
...
public static class MarketDataQueries
{
    public static IEndpointRouteBuilder MapMarketDataQueries(this IEndpointRouteBuilder endpoints)
    {
        ...
        {
            var query = new global::TomasAI.IFM.Domain.MarketData.Shared.Queries.GetFuturesOptionContractQuery(contractId);
            query = query with { Subject = new ActorSubject(ActorType.Query, global::TomasAI.IFM.Domain.MarketData.Shared.Queries.GetFuturesOptionContractQuery.Actor, global::TomasAI.IFM.Domain.MarketData.Shared.Queries.GetFuturesOptionContractQuery.Verb, query.EntityId.Format()) };
        ...
        endpoints.MapGet(MarketDataQueryUriPath.GetFuturesOptionContracts, async (IActorService e, string symbol) =>
        {
            ...
            query = query with { Subject = new ActorSubject(ActorType.Query, GetFuturesOptionContractsQuery.Actor, GetFuturesOptionContractsQuery.Verb, query.EntityId.Format()) };
        ...
        {
            var query = new GetLastYieldCurveRateQuery(true);
            query = query with { Subject = new ActorSubject(ActorType.Query, GetLastYieldCurveRateQuery.Actor, GetLastYieldCurveRateQuery.Verb, query.EntityId.Format()) };
        ...
        {
            var query = new GetYieldCurveRatesQuery(startDate, endDate);
            query = query with { Subject = new ActorSubject(ActorType.Query, GetYieldCurveRatesQuery.Actor, GetYieldCurveRatesQuery.Verb, query.EntityId.Format()) };
        ...
        {
            var query = new GetYieldCurveRateExistsQuery(valueDate);
            query = query with { Subject = new ActorSubject(ActorType.Query, GetYieldCurveRateExistsQuery.Actor, GetYieldCurveRateExistsQuery.Verb, query.EntityId.Format()) };
        ...
        {
            var query = new GetLastRateOfReturnQuery(symbol, valueDate);
            query = query with { Subject = new ActorSubject(ActorType.Query, GetLastRateOfReturnQuery.Actor, GetLastRateOfReturnQuery.Verb, query.EntityId.Format()) };
        ...
        {
            var query = new GetTradingDaysQuery(startDate, endDate, marketType, currencyType);
            query = query with { Subject = new ActorSubject(ActorType.Query, GetTradingDaysQuery.Actor, GetTradingDaysQuery.Verb, query.EntityId.Format()) };
        ...
        {
            var query = new GetTradingDatesQuery(startDate, endDate, marketType, currencyType);
            query = query with { Subject = new ActorSubject(ActorType.Query, GetTradingDatesQuery.Actor, GetTradingDatesQuery.Verb, query.EntityId.Format()) };
        ...
        endpoints.MapGet(MarketDataQueryUriPath.GetValueDate, async (IActorService e) =>
        {
            ...
            query = query with { Subject = new ActorSubject(ActorType.Query, GetValueDateQuery.Actor, GetValueDateQuery.Verb, query.EntityId.Format()) };
        ...
        endpoints.MapGet(MarketDataQueryUriPath.GetMarketSession, async (IActorService e) =>
        {
            ...
            query = query with { Subject = new ActorSubject(ActorType.Query, GetMarketSessionQuery.Actor, GetMarketSessionQuery.Verb, query.EntityId.Format()) };
...
public static class MarketDataAnalyticsQueries
{
    public static IEndpointRouteBuilder MapMarketDataAnalyticsQueries(this IEndpointRouteBuilder endpoints)
    {
        ...
        {
            var query = new GetFuturesRsiSignalQuery(contractId, valueDate, timePeriod, periodLength);
            query = query with { Subject = new ActorSubject(ActorType.Query, GetFuturesRsiSignalQuery.Actor, GetFuturesRsiSignalQuery.Verb, query.EntityId.Format()) };
        ...
        {
            var query = new GetFuturesTrendDirectionFromRSISignalQuery(
        ...
        {
            var query = new GetFuturesItiSignalMDIQuery(contractId, valueDate);
            query = query with { Subject = new ActorSubject(ActorType.Query, GetFuturesItiSignalMDIQuery.Actor, GetFuturesItiSignalMDIQuery.Verb, query.EntityId.Format()) };
        ...
        {
            var query = new GetFuturesItiSignalMDIByTrendQuery(contractId, valueDate, groupId);
            query = query with { Subject = new ActorSubject(ActorType.Query, GetFuturesItiSignalMDIByTrendQuery.Actor, GetFuturesItiSignalMDIByTrendQuery.Verb, query.EntityId.Format()) };
