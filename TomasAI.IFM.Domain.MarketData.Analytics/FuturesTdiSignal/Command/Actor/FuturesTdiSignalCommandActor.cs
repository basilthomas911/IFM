...
public class FuturesTdiSignalCommandActor(
    ...
    : BaseEventSourceCommandActor<FuturesTdiSignalCommandActor>(logger, new ActorMailboxId(ActorType.Command, ActorName))
{
    ...
    readonly IEventProjector<FuturesTdiSignalCommandActor> _eventProjector = IsArgumentNull.Set(eventProjector);
    ...
    protected override ValueTask<ServiceResult<GuidResult>> ReceiveAsync(ICommandActorContext context, IActorState state, ICommand cmd)
    {
        ...
        _ = receiveFunc.Invoke(cmd, context, tdiSignalState);
    ...
    /// </summary>
    static readonly Dictionary<string, Func<ICommand, ICommandActorContext, FuturesTdiSignalCommandState, bool>> _receiveMap = new()
    {
