    ...
    : BaseQueryActor<FuturesTdiSignalQueryActor>(actorContext.Logger, actorContext.ActorId)
{
    ...
    protected IFuturesTdiSignalQueryContext ActorContext { get; } =
        IsArgumentNull.Set(actorContext as IFuturesTdiSignalQueryContext, nameof(actorContext))!;
    ...
    /// </summary>
    static readonly Dictionary<string, Func<IQueryActorContext<FuturesTdiSignalQueryActor>, IDbContextFactory, IQuery, CancellationToken, ValueTask>> _receiveMap = new()
    {
        [typeof(GetFuturesTdiSignalQuery).Name] = async (ctx, db, q, cancellationToken) =>
        {
