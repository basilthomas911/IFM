...
using TomasAI.IFM.Domain.MarketData.Analytics.FuturesRsiSignal.Command.Extensions;

namespace TomasAI.IFM.Domain.MarketData.Analytics.FuturesRsiSignal.Command.Actor;

/// <summary>
/// Represents an actor responsible for managing futures RSI signal commands and state within an event-sourced system.
/// </summary>
/// <remarks>The <see cref="FuturesRsiSignalCommandActor"/> is a specialized command actor designed to handle operations
/// related to futures RSI signals. It processes commands such as starting, stopping, and generating RSI signals
/// (including daily signals). It validates the commands, and manages the actor's state. This actor relies on an
/// event-sourced repository for state persistence and uses dependency injection to resolve required services.</remarks>
...
public class FuturesRsiSignalCommandActor(
    ICommandActorContext<FuturesRsiSignalCommandActor> actorContext)
    : BaseEventSourceCommandActor<FuturesRsiSignalCommandActor>(actorContext, actorContext.Logger)
{
    ...
    protected override async ValueTask OnStartup(ICommandActorContext<FuturesRsiSignalCommandActor> context)
    {
    ...
    static readonly Dictionary<string, Func<IActorMessage, ICommand>> _parseMap = new()
    {
        ...
        [GenerateFuturesRsiSignalCommand.Verb] = msg => msg.AsCommand<GenerateFuturesRsiSignalCommand>()!,
        [GenerateFuturesRsiDailySignalCommand.Verb] = msg => msg.AsCommand<GenerateFuturesRsiDailySignalCommand>()!
    };
    ...
    /// <param name="state">The current state of the actor. Must be a valid instance of <see cref="FuturesRsiSignalCommandState"/>. Cannot be null.</param>
    ...
    protected override ValueTask<ServiceResult<GuidResult>> ReceiveAsync(ICommandActorContext<FuturesRsiSignalCommandActor> context, IActorState state, ICommand cmd)
    {
        ...
        _ = receiveFunc.Invoke(cmd, dispatchContext, rsiSignalState);
    ...
    /// </summary>
    static readonly Dictionary<string, Func<ICommand, ICommandActorContext<FuturesRsiSignalCommandActor>, FuturesRsiSignalCommandState, bool>> _receiveMap = new()
    {
    ...
    protected override ValueTask OnValidateAsync(ICommandActorContext<FuturesRsiSignalCommandActor> context, ActorThreadId threadId, ICommand cmd)
        => OnValidateAsync(context, threadId, cmd, CancellationToken.None);

    protected override async ValueTask OnValidateAsync(ICommandActorContext<FuturesRsiSignalCommandActor> context, ActorThreadId threadId, ICommand cmd, CancellationToken cancellationToken)
    {
        IsArgumentNull.Check(context);
    ...
    protected override async ValueTask<IActorState> OnLoadStateAsync(ICommandActorContext<FuturesRsiSignalCommandActor> context, ActorThreadId threadId, ICommand cmd)
    {
        IsArgumentNull.Check(context);
        IsArgumentNull.Check(threadId);
        IsArgumentNull.Check(cmd);
    ...
    /// <summary>
    /// Asynchronously saves the current state of the futures RSI signal actor in response to a command.
    /// </summary>
    ...
    /// cref="FuturesRsiSignalCommandState"/>.</param>
    ...
    protected override async ValueTask OnSaveStateAsync(ICommandActorContext<FuturesRsiSignalCommandActor> context, ActorThreadId threadId, IActorState state, ICommand cmd)
    {
        IsArgumentNull.Check(context);
        IsArgumentNull.Check(threadId);
        IsArgumentNull.Check(state);
        IsArgumentNull.Check(cmd);
        var rsiSignalState = IsArgumentNull.Set((state as FuturesRsiSignalCommandState)!);
        await _repo.SaveStateAsync(context, rsiSignalState, cmd);
    }
    ...
    protected override async ValueTask<IActorState> OnLoadStateAsync(ICommandActorContext<FuturesRsiSignalCommandActor> context, ActorThreadId threadId, ICommand cmd, CancellationToken cancellationToken)
        => await _repo.LoadStateAsync(cmd, cancellationToken).ConfigureAwait(false);

    protected override async ValueTask OnSaveStateAsync(ICommandActorContext<FuturesRsiSignalCommandActor> context, ActorThreadId threadId, IActorState state, ICommand cmd, CancellationToken cancellationToken)
        => await _repo.SaveStateAsync(context, (FuturesRsiSignalCommandState)state, cmd, cancellationToken).ConfigureAwait(false);

    protected override async ValueTask<ServiceResult<GuidResult>> OnExceptionAsync(ICommandActorContext<FuturesRsiSignalCommandActor> context, ActorThreadId threadId, ICommand command, Exception ex)
    {
        try
        {
            IsArgumentNull.Check(context);
