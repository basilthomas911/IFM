...
using TomasAI.IFM.Domain.MarketData.Analytics.FuturesMacdSignal.Command.Extensions;

namespace TomasAI.IFM.Domain.MarketData.Analytics.FuturesMacdSignal.Command.Actor;

/// <summary>
/// Represents an actor responsible for handling futures MACD signal commands within an event-sourced system.
/// </summary>
/// <param name="dbEventSource">The event source database context used for logging and persisting command events.</param>
/// <param name="dbFactory">The database context factory used to create database contexts.</param>
...
public class FuturesMacdSignalCommandActor(
    ICommandActorContext<FuturesMacdSignalCommandActor> actorContext)
    : BaseEventSourceCommandActor<FuturesMacdSignalCommandActor>(actorContext, actorContext.Logger)
{
    /// <summary>Gets the domain-specific typed context owned by this actor.</summary>
    protected IFuturesMacdSignalCommandContext ActorContext { get; } =
        IsArgumentNull.Set(actorContext as IFuturesMacdSignalCommandContext, nameof(actorContext))!;
    ...
    readonly IEventProjector<FuturesMacdSignalCommandActor> _eventProjector = IsArgumentNull.Set(actorContext.EventProjector);
    ...
    protected override async ValueTask OnStartup(ICommandActorContext<FuturesMacdSignalCommandActor> context)
    {
    ...
    protected override async ValueTask OnShutdown(ICommandActorContext<FuturesMacdSignalCommandActor> context)
    ...
    protected override ICommand ParseMessage(ICommandActorContext<FuturesMacdSignalCommandActor> context, IActorMessage message)
    {
        IsArgumentNull.Check(context);
    ...
    /// <param name="state">The current state of the actor. Must be a valid instance of <see cref="FuturesMacdSignalCommandState"/>. Cannot be null.</param>
    ...
    protected override ValueTask<ServiceResult<GuidResult>> ReceiveAsync(ICommandActorContext<FuturesMacdSignalCommandActor> context, IActorState state, ICommand cmd)
    {
        var dispatchContext = context;
        IsArgumentNull.Check(context);
        IsArgumentNull.Check(state);
        IsArgumentNull.Check(cmd);
        var macdSignalState = IsArgumentNull.Set((state as FuturesMacdSignalCommandState)!);
    ...
    protected override ValueTask OnValidateAsync(ICommandActorContext<FuturesMacdSignalCommandActor> context, ActorThreadId threadId, ICommand cmd)
        => OnValidateAsync(context, threadId, cmd, CancellationToken.None);

    protected override async ValueTask OnValidateAsync(ICommandActorContext<FuturesMacdSignalCommandActor> context, ActorThreadId threadId, ICommand cmd, CancellationToken cancellationToken)
    {
        IsArgumentNull.Check(context);
    ...
    protected override async ValueTask<IActorState> OnLoadStateAsync(ICommandActorContext<FuturesMacdSignalCommandActor> context, ActorThreadId threadId, ICommand cmd)
    {
        IsArgumentNull.Check(context);
        IsArgumentNull.Check(threadId);
        IsArgumentNull.Check(cmd);
    ...
    /// <summary>
    /// Asynchronously saves the current state of the futures MACD signal actor in response to a command.
    /// </summary>
    ...
    /// cref="FuturesMacdSignalCommandState"/>.</param>
    ...
    protected override async ValueTask OnSaveStateAsync(ICommandActorContext<FuturesMacdSignalCommandActor> context, ActorThreadId threadId, IActorState state, ICommand cmd)
    {
        IsArgumentNull.Check(context);
        IsArgumentNull.Check(threadId);
        IsArgumentNull.Check(state);
        IsArgumentNull.Check(cmd);
        var macdSignalState = IsArgumentNull.Set((state as FuturesMacdSignalCommandState)!);
        await _repo.SaveStateAsync(context, macdSignalState, cmd);
    }
    ...
    protected override async ValueTask<IActorState> OnLoadStateAsync(ICommandActorContext<FuturesMacdSignalCommandActor> context, ActorThreadId threadId, ICommand cmd, CancellationToken cancellationToken)
        => await _repo.LoadStateAsync(cmd, cancellationToken).ConfigureAwait(false);

    protected override async ValueTask OnSaveStateAsync(ICommandActorContext<FuturesMacdSignalCommandActor> context, ActorThreadId threadId, IActorState state, ICommand cmd, CancellationToken cancellationToken)
        => await _repo.SaveStateAsync(context, (FuturesMacdSignalCommandState)state, cmd, cancellationToken).ConfigureAwait(false);

    protected override async ValueTask<ServiceResult<GuidResult>> OnExceptionAsync(ICommandActorContext<FuturesMacdSignalCommandActor> context, ActorThreadId threadId, ICommand command, Exception ex)
    {
        try
        {
            IsArgumentNull.Check(context);
