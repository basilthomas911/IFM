...
public class FuturesMacdSignalQueryActor(
    ...
    : BaseQueryActor<FuturesMacdSignalQueryActor>(logger, new ActorMailboxId(ActorType.Query, ActorName))
{
    ...
    /// <param name="state">The current state of the actor, which must be of type <see cref="FuturesMacdSignalQueryState"/>.</param>
    ...
    {
        [typeof(GetFuturesMacdSignalQuery).Name] = async (ctx, dbFactory, q) =>
        {
            var query = (q as GetFuturesMacdSignalQuery)!;
            var result = await query.GetLastFuturesMacdSignalAsync(dbFactory);
            var serviceResult = new ServiceResult<FuturesMacdSignalReadModel>(result);
            await ctx.ReplyAsync(q.Subject.ThreadId, GetFuturesMacdSignalQuery.Verb, serviceResult);
        },
        [typeof(GetFuturesMacdDailySignalQuery).Name] = async (ctx, dbFactory, q) =>
        {
            var query = (q as GetFuturesMacdDailySignalQuery)!;
            var result = await query.GetLastFuturesMacdDailySignalAsync(dbFactory);
            var serviceResult = new ServiceResult<FuturesMacdSignalReadModel>(result);
            await ctx.ReplyAsync(q.Subject.ThreadId, GetFuturesMacdDailySignalQuery.Verb, serviceResult);
        }
    };
