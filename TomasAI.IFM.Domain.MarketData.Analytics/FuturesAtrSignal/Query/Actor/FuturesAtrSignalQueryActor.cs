    ...
    : BaseQueryActor<FuturesAtrSignalQueryActor>(actorContext, actorContext.Logger)
{
    ...
    /// </summary>
    static readonly Dictionary<string, Func<IQueryActorContext<FuturesAtrSignalQueryActor>, IDbContextFactory, IQuery, CancellationToken, ValueTask>> _receiveMap = new()
    {
        [typeof(GetFuturesAtrSignalQuery).Name] = async (ctx, dbFactory, q, cancellationToken) =>
        {
        ...
        [typeof(GetFuturesAtrDailySignalQuery).Name] = async (ctx, dbFactory, q, cancellationToken) =>
        {
    ...
    protected override async ValueTask OnExceptionAsync(IQueryActorContext<FuturesAtrSignalQueryActor> context, ActorThreadId threadId, IQuery query, string verb, Exception ex)
    {
        IsArgumentNull.Check(context);
