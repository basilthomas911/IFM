...
/// <summary>
/// Represents an actor responsible for managing futures ATR signal commands and state within an event-sourced system.
/// </summary>
/// <remarks>The <see cref="FuturesAtrSignalCommandActor"/> is a specialized command actor designed to handle operations
/// related to futures ATR signals. It processes the generate command, validates the command, and manages the actor's state.
...
public class FuturesAtrSignalCommandActor(
    ...
    : BaseEventSourceCommandActor<FuturesAtrSignalCommandActor>(actorContext, actorContext.Logger)
{
    /// <summary>Gets the domain-specific typed context owned by this actor.</summary>
    protected IFuturesAtrSignalCommandContext ActorContext =>
    ...
    protected override async ValueTask OnStartup(ICommandActorContext<FuturesAtrSignalCommandActor> context)
    {
        IsArgumentNull.Check(context);
        _repo = IsArgumentNull.Set(context.Container.Resolve<IEventSourceActorStateRepository<FuturesAtrSignalCommandState>>());
        await EventProjector.StartAsync(context).ConfigureAwait(false);
    }

    protected override async ValueTask OnShutdown(ICommandActorContext<FuturesAtrSignalCommandActor> context)
    ...
    /// <param name="state">The current state of the actor. Must be a valid instance of <see cref="FuturesAtrSignalCommandState"/>. Cannot be null.</param>
    ...
    protected override ValueTask<ServiceResult<GuidResult>> ReceiveAsync(ICommandActorContext<FuturesAtrSignalCommandActor> context, IActorState state, ICommand cmd)
    {
    ...
    protected override ValueTask OnValidateAsync(ICommandActorContext<FuturesAtrSignalCommandActor> context, ActorThreadId threadId, ICommand cmd)
        => OnValidateAsync(context, threadId, cmd, CancellationToken.None);

    protected override async ValueTask OnValidateAsync(ICommandActorContext<FuturesAtrSignalCommandActor> context, ActorThreadId threadId, ICommand cmd, CancellationToken cancellationToken)
    {
        IsArgumentNull.Check(context);
    ...
    protected override async ValueTask<IActorState> OnLoadStateAsync(ICommandActorContext<FuturesAtrSignalCommandActor> context, ActorThreadId threadId, ICommand cmd)
    {
        IsArgumentNull.Check(context);
        IsArgumentNull.Check(threadId);
        IsArgumentNull.Check(cmd);
    ...
    /// <summary>
    /// Asynchronously saves the current state of the futures ATR signal actor in response to a command.
    /// </summary>
    ...
    /// cref="FuturesAtrSignalCommandState"/>.</param>
    ...
    protected override async ValueTask OnSaveStateAsync(ICommandActorContext<FuturesAtrSignalCommandActor> context, ActorThreadId threadId, IActorState state, ICommand cmd)
    {
        IsArgumentNull.Check(context);
        IsArgumentNull.Check(threadId);
        IsArgumentNull.Check(state);
        IsArgumentNull.Check(cmd);
        var atrSignalState = IsArgumentNull.Set((state as FuturesAtrSignalCommandState)!);
        await _repo.SaveStateAsync(context, atrSignalState, cmd);
    }
    ...
    protected override async ValueTask<IActorState> OnLoadStateAsync(ICommandActorContext<FuturesAtrSignalCommandActor> context, ActorThreadId threadId, ICommand cmd, CancellationToken cancellationToken)
        => await _repo.LoadStateAsync(cmd, cancellationToken).ConfigureAwait(false);

    protected override async ValueTask OnSaveStateAsync(ICommandActorContext<FuturesAtrSignalCommandActor> context, ActorThreadId threadId, IActorState state, ICommand cmd, CancellationToken cancellationToken)
        => await _repo.SaveStateAsync(context, (FuturesAtrSignalCommandState)state, cmd, cancellationToken).ConfigureAwait(false);

    protected override async ValueTask<ServiceResult<GuidResult>> OnExceptionAsync(ICommandActorContext<FuturesAtrSignalCommandActor> context, ActorThreadId threadId, ICommand command, Exception ex)
    {
        try
        {
            IsArgumentNull.Check(context);
