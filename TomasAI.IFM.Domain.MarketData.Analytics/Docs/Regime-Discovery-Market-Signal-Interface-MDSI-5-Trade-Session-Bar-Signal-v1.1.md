...
Futures Trade Session Bar Signal v1.1
...
MDSI-5 now uses the business-specific name `FuturesTradeSessionBarSignal`.
...
FuturesMarketPriceUpdatedRealtimeEvent
    -> FuturesTradeSessionBarSignalRealtimeActor
    ...
    -> FuturesTradeSessionBarSignalEventProjector
    -> ScyllaDB: futures_trade_session_bar
    -> FuturesTradeSessionBarPublishedCompleteEvent
    -> FuturesTradeSessionBarSignalEventActor
...
`FuturesTradeSessionBarSignalRealtimeActor` owns route and timer lifecycle
...
- `FuturesTradeSessionBarSignalBarrierRealtimeEvent` invokes the elapsed-bar
  handler.
...
`FuturesTradeSessionBarSignalEventProjector`, located under `Command`,
