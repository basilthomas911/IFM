...
public sealed class MarketConditionAssessmentConfigurationTests(MarketConditionConfigurationDbFixture fixture)
{
    ...
    public async Task Published_profile_resolves_only_its_market_and_horizon_and_retirement_preserves_exact_version(TimeFrameType horizon)
    {
        ...
        (await fixture.Context.GetEffectiveMarketConditionAssessmentAsync(at,p.MarketProfileId,"ES",horizon)).Should().BeNull();
        ...
        var resolved = await fixture.Context.GetEffectiveMarketConditionAssessmentAsync(at,p.MarketProfileId,"ES",horizon);
        resolved!.ParameterSet.Should().BeEquivalentTo(p);
        resolved.PayloadSha256.Should().Be(MarketConditionAssessmentHash.Parameters(p));
        (await fixture.Context.GetEffectiveMarketConditionAssessmentAsync(at,p.MarketProfileId+"other","ES",horizon)).Should().BeNull();
        var other = horizon == TimeFrameType.Daily ? TimeFrameType.Weekly : TimeFrameType.Daily;
        (await fixture.Context.GetEffectiveMarketConditionAssessmentAsync(at,p.MarketProfileId,"ES",other)).Should().BeNull();
        await fixture.Context.RetireAsync(StrategyParameterSetKind.MarketConditionAssessment,p.ParameterSetId,p.Version,at);
        (await fixture.Context.GetEffectiveMarketConditionAssessmentAsync(at.AddSeconds(1),p.MarketProfileId,"ES",horizon)).Should().BeNull();
    ...
    public async Task Overlapping_published_versions_are_rejected_instead_of_selecting_arbitrarily()
    {
        ...
        Func<Task> resolve = async () => await fixture.Context.GetEffectiveMarketConditionAssessmentAsync(at,p.MarketProfileId,"ES",p.TargetHorizon);
    ...
    public async Task Future_publication_is_not_effective_and_delete_is_forbidden()
    {
        ...
        (await fixture.Context.GetEffectiveMarketConditionAssessmentAsync(now,p.MarketProfileId,"ES",p.TargetHorizon)).Should().BeNull();
