...
`G1NavigationAndQueryAuditTests` uses the same ownership and evidence infrastructure for the 15-step navigation/read-only gate. It establishes expected selector and row state through typed NATS queries, then proves the real shell, status history, ES/VX charts, calendar ranges, supported modal catalogs, normal close, and bounded cleanup through UI Automation. G1 selects a bounded 20-record deterministic synthetic tick source at two records per second so both charts render a meaningful line without native-provider throughput obscuring the read-only UI decision or starving UI Automation; G0 remains the live DataBento startup/feed gate. `G2PrerequisiteAndStartupAuditTests` owns the reversible command catalog; it currently executes G2-001 through G2-034 or one of its accepted isolated command slices. G4 supplies a complete bounded Synthetic futures catalog because its process audit qualifies lifecycle resilience rather than external-provider behavior. Only one process gate may run at a time.
...
$env:IFM_G2_LOOKUP_SLICE = $null
$env:IFM_G2_FUND_SLICE = '1'
$env:IFM_G2_RETAIN_FUND_FIXTURE = '1' # required only to approve first-time retained fixture creation
dotnet test TomasAI.IFM.UI.Net.SystemTests/TomasAI.IFM.UI.Net.SystemTests.csproj --no-build --filter Category=G2StartupProcess

$env:IFM_G2_FUND_SLICE = $null
$env:IFM_G2_ORDER_TRADE_SLICE = '1'
dotnet test TomasAI.IFM.UI.Net.SystemTests/TomasAI.IFM.UI.Net.SystemTests.csproj --no-build --filter Category=G2StartupProcess
```
...
Every isolated G2 domain slice overrides only the unrelated streaming source with the bounded `Synthetic` adapter. The unsliced G2 audit and G0 retain the Development live-provider configuration.
...
The isolated lookup slice executes G2-001 through G2-007 and G2-024 through G2-026. It creates a brand-new run-prefixed lookup partition containing one order-zero value so removal cannot reorder operator-owned lookup data. The harness proves add/change/remove through the real editor, exact source/terminal command IDs, typed partition queries, and refreshed lookup-name/short-code selectors. Cleanup uses the public Reference command API and verifies the partition equals its captured empty baseline.

The isolated fund slice executes G2-001 through G2-007 and G2-027 through G2-029. It overrides only the unrelated market-data source to `Synthetic`; fund UI, actor, event, query, and storage paths remain the real Development implementations. It resolves the named non-production fixture from `IFM_G2_FUND_NAME` (default `IFM G2 Automation Fund`). If absent, creation is blocked unless `IFM_G2_RETAIN_FUND_FIXTURE=1` explicitly approves creating that reusable fund through the real UI/domain command; the initial balance defaults to `100000` and can be changed with `IFM_G2_FUND_INITIAL_BALANCE`. The run then submits a run-referenced cash deposit and equal withdrawal from the Fund Transactions editor, correlates each source and terminal event by exact command ID, confirms immutable durable/UI rows, and proves the original balance is restored. `IFM_G2_FUND_TRANSACTION_AMOUNT` controls the positive amount (default `123.45`). Finally-path reconciliation submits the missing compensating withdrawal through the public Fund command API if interruption occurs after the deposit.

The isolated order/trade slice executes G2-001 through G2-007, G2-027, and G2-030 through G2-034. It reuses the same named non-production fund and synthetic override for unrelated streaming, then creates one run-referenced order and one Short Iron Condor trade through the real Trade Orders editor. The modal-displayed trade ID must be the identity sent to the domain actor and rendered after refresh. The slice correlates exact source/terminal command IDs for order add, trade add, trade-state change, child removal, and parent removal; typed durable queries and visible rows must agree after every transition. Cleanup is child-first and verifies the captured order/trade baseline. `IFM_G2_SECURITIES_SLICE`, `IFM_G2_YIELD_CURVE_SLICE`, `IFM_G2_ECONOMIC_CALENDAR_SLICE`, `IFM_G2_LOOKUP_SLICE`, `IFM_G2_FUND_SLICE`, and `IFM_G2_ORDER_TRADE_SLICE` are mutually exclusive.
