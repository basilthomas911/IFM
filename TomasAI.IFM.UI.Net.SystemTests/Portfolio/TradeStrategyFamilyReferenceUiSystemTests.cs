...
public sealed class TradeStrategyFamilyReferenceUiSystemTests
{
    ...
    static async Task VerifyReferenceFormAsync()
    {
        ...
        queries.QueryStrategyCatalogAsync(Arg.Any<CatalogQueryRequest>(), Arg.Any<CancellationToken>())
            .Returns(new ServiceOk<string>(StrategyCatalogJson.Write(Array.Empty<StrategyCatalogSummary>())));
        queries.GetTradeStrategySymbolsAsync(Arg.Any<TradeStrategyFamilyType>(), Arg.Any<CancellationToken>())
            .Returns(new ServiceOk<TradeStrategySymbolReadModel[]>([new() { Id = 101, Symbol = "ES", Currency = "USD", Exchange = "XCME", Description = "ES futures" }]));
        ...
        var view = (StrategyCatalogReferenceView)Field<Panel>(form, "pnlMarketData").Controls[0];
        await view.LoadAsync();
        PrepareForDisplay(form);
        AssertFont(form);
        Field<Button>(form, "btnAdd").Enabled.Should().BeTrue();
        foreach (var name in new[] { "btnChange", "btnRemove" }) Field<Button>(form, name).Enabled.Should().BeFalse();
        ...
        Field<Button>(form, "btnAdd").Text.Should().Be("Save"); Field<Button>(form, "btnAdd").Enabled.Should().BeTrue();
