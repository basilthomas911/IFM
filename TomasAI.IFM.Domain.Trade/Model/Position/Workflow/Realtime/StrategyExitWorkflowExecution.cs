...
{
    /// <summary>Projects workflow progress, composes the close, obtains Portfolio approval, and dispatches an accepted order.</summary>
    /// <param name="started">The durable strategy exit-workflow start.</param>
    /// <param name="context">The realtime actor context used for function requests and order dispatch.</param>
    /// <param name="timeProvider">The clock used for deterministic workflow timestamps.</param>
    /// <param name="tradePlanDb">The Trade Plan projection writer.</param>
    /// <returns>A task that completes after the workflow reaches no-trade or dispatches its accepted order.</returns>
