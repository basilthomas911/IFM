...
public sealed class TradeOrderEventProjector : ConventionalEventProjector<TradeOrderCommandActor>
{
            ...
            Typed(actorContext).BlackboardService,Typed(actorContext).Logger,options)
    {
        ...
        descriptors = [DescribeNotification<TradeOrderChangedEvent, TomasAI.IFM.Domain.Trade.Shared.Model.TradeOrderId>(ProjectAsync)];
    }
    ...
    async Task ProjectAsync(TradeOrderChangedEvent changed)
    {
        ...
        if (changed.State.Status != TomasAI.IFM.Domain.Trade.Shared.Model.TradeOrderStatus.Executing || changed.ExecutionAttemptId == Guid.Empty) return;
        var executionId = new TomasAI.IFM.Domain.Trade.Shared.Model.OrderExecutionId(
        ...
        var result = await context.ActorService.SendAsync<StartOrderExecutionCommand, TomasAI.IFM.Domain.Trade.Shared.Model.OrderExecutionId>(command,command.EntityId).ConfigureAwait(false);
