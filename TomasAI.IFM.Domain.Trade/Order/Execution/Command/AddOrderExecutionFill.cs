...
public static class AddOrderExecutionFill
{
    /// <summary>Adds one correlated fill or ignores its exact durable replay.</summary>
    public static ServiceResult<GuidResult> Execute(this AddOrderExecutionFillCommand command,
        OrderExecutionCommandState state) => OrderExecutionCommandModel.Apply(command, state,
        machine => machine.AddFill(command.Fill));
}
