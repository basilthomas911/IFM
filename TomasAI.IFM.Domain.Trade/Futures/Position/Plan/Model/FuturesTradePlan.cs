...
namespace TomasAI.IFM.Domain.Trade.Futures.Position.Plan.Model;

public sealed record FuturesTradePlan(StrategyTradePlanSnapshot Snapshot);

public sealed class FuturesTradePlanAlgorithm
{
    public FuturesTradePlan Calculate(StrategyPositionSnapshot position, TradePlanParameters parameters,
        StrategyTradePlanSnapshot? previous, DateOnly valueDate, DateTime nowUtc)
    {
        FuturesTradePlanValidation.Validate(position, parameters, valueDate, nowUtc);
        var leg = position.Legs[0];
        var opening = leg.OpeningPrice * leg.SignedQuantity;
        var current = leg.CurrentPrice * leg.SignedQuantity;
        var pnl = position.RealizedPnl + position.UnrealizedPnl;
        var forward = FuturesForwardTradePriceModel.Calculate(current, previous?.CurrentValue);
        var decision = FuturesExitConditionModel.Evaluate(position, pnl, forward - current, parameters, nowUtc);
        ...
        plan = plan with { MaterialChange = FuturesTradePlanComparisonModel.IsMaterial(previous, plan) };
        return new(plan with { ContentHash = TradePlanContractIdentity.PlanHash(plan) });
    }
}

public static class FuturesForwardTradePriceModel
{
...
public static class FuturesExitConditionModel
{
    public static FuturesTradePlanDecision Evaluate(StrategyPositionSnapshot position, decimal pnl,
...
public static class FuturesTradePlanComparisonModel
{
...
public static class FuturesTradePlanValidation
{
    public static void Validate(StrategyPositionSnapshot position, TradePlanParameters parameters, DateOnly valueDate, DateTime nowUtc)
    {
        ...
        if (!position.Id.IsValid || position.StrategyKind != TradeStrategyKind.FuturesOutright || position.Legs.Length != 1 ||
            ...
            throw new ArgumentException("A valid one-leg Futures position snapshot is required.");
    }
}

public readonly record struct FuturesTradePlanDecision(TradePlanState State, TradePlanAction Action,
