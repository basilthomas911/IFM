    ...
    ICompositionRoutePlanStore plans, string authorityScope = "IFM") : ICommittedBusinessSubscriptionSource
{
    ...
    public async Task<DurableAuthorityMutation?> ReadAsync(BusinessSubscriptionSourceReference reference, CancellationToken cancellationToken)
    {
        ...
        if (value is WorkflowStrategyStateUpdatedEvent workflow)
        {
            ...
            if (reference.Kind != BusinessSubscriptionSourceKind.IntrinsicTimeWorkflow)
        ...
        else
        {
            ...
            {
                var prior = await journal.ReadPriorAsync(row.EventStreamId, row.EventVersion, TradeSnapshotNames, cancellationToken).ConfigureAwait(false);
            ...
            status = reference.Kind == BusinessSubscriptionSourceKind.TradePosition
                ? value switch
                {
        ...
        if (entity != reference.EntityId) throw new InvalidDataException("Source entity does not match the committed event.");
        if (status == DurableAuthorityStatus.Active && selected is null) status = DurableAuthorityStatus.Unknown;
        var purpose = reference.Kind switch
        {
        ...
        var sourceId = reference.Kind + ":" + entity;
        var owner = new DurableSubscriptionOwner(reference.Kind.ToString(), entity, "SelectedContracts");
        ...
        if (reference.Kind == BusinessSubscriptionSourceKind.TradeOrder && value is OptionTradePositionOpenedEvent
        ...
        return DurableSubscriptionContract.Freeze(new DurableAuthorityMutation(authorityScope, "GLBX.MDP3",
            ...
            sourceId, version, value.Id, owner, status, "Committed" + status, adds, [], CompleteSourceSnapshot: true));
    }
