    ...
    ICompositionMarketDataApi market, IDurableCompositionReconciler runtime)
{
    public async Task CompletePendingAsync(CancellationToken cancellationToken)
    {
        foreach (var row in await journal.ReadPendingHandoffsAsync(cancellationToken).ConfigureAwait(false))
        {
            ...
            await CompleteAsync(workflow, row.EventVersion,
                row.StreamVersion > 0 ? row.StreamVersion : row.EventVersion, cancellationToken).ConfigureAwait(false);
        }
    }

    public async Task CompleteAsync(WorkflowStrategyStateUpdatedEvent workflow, long eventId,
        long sourceVersion, CancellationToken cancellationToken)
    {
        ...
        if (evidence is not null && (selected is not null || terminal))
        {
            ...
            if (owner is null || owner.SourceVersion < sourceVersion) return;
        ...
        await journal.CompleteHandoffAsync(eventId, cancellationToken).ConfigureAwait(false);
    }
}
