...
`ExecuteMarketConditionAssessment` captures a snapshot and invokes the model, then dispatches its outcome through the actor callback. `CompleteMarketConditionAssessment` and `FailMarketConditionAssessment` build terminal candidates through `_eventMap`. Lifecycle exceptions, conflicting duplicates, and workflow transport failures use the same dispatcher. The completion extension also receives the already committed event through that map to record success telemetry; it does not create another event or persist it. Matching replay returns the original event directly without re-running calculations, projection, or telemetry.

The base Function actor owns lifecycle deadline enforcement. Market Condition supplies the domain clock and a stage deadline: loading receives a fresh bounded read budget so completed replay remains possible after market expiry; execution, projection, and persistence use the frozen request deadline. Exact-boundary timeout wins, caller cancellation remains distinct, and late dependencies are cancelled and observed without advancing subsequent stages. No separate deadline helper class is introduced. Other Function actors retain their existing timing policy unless they opt into this base deadline policy.

New completions carry `StrategyStageResultEnvelope.AssessmentResult` at appended MessagePack key 9, leaving `Payload` empty. Outer message/event-store serialization handles the typed graph. Canonical assessment JSON is used only to compute a content fingerprint and the 64-KiB content budget; no encoded inner result is embedded. This uses the existing assessment numeric-normalization rules. Typed getters defensively copy mutable collections. Existing keys 0?8 retain their meanings, null typed fields are omitted from JSON, and legacy byte envelopes remain readable. Update producers and consumers together: older consumers cannot read new typed-only assessment envelopes. Workflow and Trade Selection readers validate typed results directly. Scylla continues storing the serialized outer completion and its blob checksum at the storage boundary; no table migration is required.

The `Assess` route, AssessmentV2 execution identity, request schema, calculation thresholds, single triggering Daily/Weekly/Monthly horizon, and projection-before-persistence ordering remain authoritative. Request fingerprints preserve historical trigger string and timestamp normalization through typed operations. Failed results are never saved as Function state. The synchronous Function projector owns no queue or durable-event subscription; the workflow command actor owns durable acceptance/failure.

See the [actor](../Function/Actor/MarketConditionFunctionActor.cs), [execution handler](../Function/ExecuteMarketConditionAssessment.cs), [completion handler](../Function/CompleteMarketConditionAssessment.cs), and [failure handler](../Function/FailMarketConditionAssessment.cs). Current verification is recorded in [Gate Evidence](MarketCondition-Gate-Evidence-v2.0.md).
