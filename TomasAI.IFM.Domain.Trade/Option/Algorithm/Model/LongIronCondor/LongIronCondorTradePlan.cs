...
public class LongIronCondorTradePlan : TradePlan, ITradeAlgorithm
{
    ...
    public LongIronCondorTradePlan(DateOnly valueDate, IOptionTradeCollection optionTrades, FuturesEodDataV2ReadModel futuresEodData, FuturesTradeSignalV2ReadModel futuresTradeSignal, IBlackboardService blackboardService)
        : base(
            ...
            forwardLossRatio: optionTrades.PrimaryTrade is null ? 0.0 : optionTrades.PrimaryTrade.TradePositions?.GetFowardLossRatio(optionTrades.PrimaryTrade.TradeType, TradeStatus.IntraDay, optionTrades.PrimaryTrade.TradeLimit?.MinProfitLimit ?? 0m) ?? 0.0,
            lossProbability: optionTrades.PrimaryTrade is null ? 0.0 : optionTrades.PrimaryTrade.TradePositions?.Get(optionTrades.PrimaryTrade.TradeType, OptionType.Put, TradeStatus.IntraDay)?.LossProbability ?? 0.0,
            mscore: 0,
            ...
            fiftyDayMA: 0,
            fiveDayXMA: 0,
            putOTMProbability: optionTrades.PrimaryTrade is null ? 0.0 : optionTrades.PrimaryTrade.TradePositions?.Get(optionTrades.PrimaryTrade.TradeType, OptionType.Put, TradeStatus.IntraDay)?.OTMProbability ?? 0.0,
            callOTMProbability: optionTrades.PrimaryTrade is null ? 0.0 : optionTrades.PrimaryTrade.TradePositions?.Get(optionTrades.PrimaryTrade.TradeType, OptionType.Call, TradeStatus.IntraDay)?.OTMProbability ?? 0.0,
            shortPutGamma: optionTrades.PrimaryTrade is null ? 0.0 : optionTrades.PrimaryTrade.TradePositions?.Get(optionTrades.PrimaryTrade.TradeType, OptionType.Put, TradeStatus.IntraDay)?.OptionLegData.Get(OptionLegAction.Short, OptionType.Put)?.Gamma ?? 0.0,
            shortCallGamma: optionTrades.PrimaryTrade is null ? 0.0 : optionTrades.PrimaryTrade.TradePositions?.Get(optionTrades.PrimaryTrade.TradeType, OptionType.Call, TradeStatus.IntraDay)?.OptionLegData.Get(OptionLegAction.Short, OptionType.Call)?.Gamma ?? 0.0,
            gammaRisk: GammaRiskType.None,
            netPrice: optionTrades.PrimaryTrade is null ? 0m : optionTrades.PrimaryTrade.TradePositions?.GetNetSpread(optionTrades.PrimaryTrade.TradeType, TradeStatus.IntraDay) ?? 0m,
            forwardPrice: optionTrades.PrimaryTrade is null ? 0m : optionTrades.PrimaryTrade.TradePositions?.GetForwardPrice(optionTrades.PrimaryTrade.TradeType, TradeStatus.IntraDay) ?? 0m,
            forwardDelta: 0.0,
            stopLossLimit: 0.0)
    {
        ...
        var ironCondorMDILimit = new IronCondorMDILimitDataModel(
            Id: optionTrades.PrimaryTrade?.EntityId ?? new(OrderId, TradeId),
            ValueDate: valueDate,
    ...
    public static TradePlan? Create(DateOnly valueDate, IOptionTradeCollection optionTrades, FuturesEodDataV2ReadModel futuresEodData, FuturesTradeSignalV2ReadModel futuresTradeSignal, IBlackboardService blackboardService)
    {
        ...
        if (ironCondorTrades.Count > 0)
            return new LongIronCondorAlgorithm(valueDate, ironCondorTrades, futuresEodData, futuresTradeSignal, blackboardService);
        return default;
    }
    ...
    public LongIronCondorTradePlan SetForwardLossLimitType(Func<int, int, TradeType, DateOnly,ForwardLossLimitType> getForwardLossLimitType)
    {
