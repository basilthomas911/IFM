...
public sealed class LegacyFinancialWriterFence(IPostgresEventTransaction transactions)
{
    public Task<Guid> BeginWriteAsync(IEnumerable<int> fundIds,CancellationToken token=default)
    {
        ...
        return transactions.ExecuteAsync(async(db,ct)=>
        {
            foreach(var fund in funds)
            {
                ...
                Require(await db.ScalarAsync(PortfolioDbSql.Financial.LegacyFinancialWriterFence.Select01,[fund],ct) is "Legacy",
                    FinancialReasons.AuthorityDenied,"Legacy transaction writes are fenced for this Fund.");
                await db.ExecuteAsync(PortfolioDbSql.Financial.LegacyFinancialWriterFence.Insert01,[ticket,fund],ct);
            }
            return ticket;
        },token);
    }
    ...
    {
        await db.ExecuteAsync(PortfolioDbSql.Financial.LegacyFinancialWriterFence.Update01,[ticket],ct);
        return true;
    },token);
    ...
    public Task FreezeFreshScopeAsync(int portfolioId,int fundId,Guid qualificationId,CancellationToken token=default)
    {
        ...
        return transactions.ExecuteAsync(async(db,ct)=>
        {
            ...
            var state=(await db.QueryAsync(PortfolioDbSql.Financial.LegacyFinancialWriterFence.Select02,[fundId],
            ...
            Require(await db.ScalarAsync(PortfolioDbSql.Financial.LegacyFinancialWriterFence.Select03,[fundId],ct) is 0L,
            ...
            Require(await db.ScalarAsync(PortfolioDbSql.Financial.LegacyFinancialWriterFence.Select04,[prefix,fundPrefix,fundPrefix+".%"],ct) is 0L,
                FinancialReasons.AuthorityDenied,"Existing legacy event history requires a reconciled migration.");
            await db.ExecuteAsync(PortfolioDbSql.Financial.LegacyFinancialWriterFence.Update02,[fundId,portfolioId,qualificationId],ct);
            return true;
        },token);
    }
    ...
    {
        await db.ScalarAsync(PortfolioDbSql.Financial.LegacyFinancialWriterFence.Select05,[fundId],ct);
        await db.ExecuteAsync(PortfolioDbSql.Financial.LegacyFinancialWriterFence.Insert02,[fundId],ct);
    }
    ...
    public Task<long> FreezeRetainedScopeAsync(int portfolioId,int fundId,Guid retentionId,CancellationToken token=default)
    {
        ...
        {
            await LockScope(db,fundId,ct);
            var state=(await db.QueryAsync(PortfolioDbSql.Financial.LegacyFinancialWriterFence.Select06,[fundId],
                r=>(State:r.GetString(0),Portfolio:r.IsDBNull(1)?0:r.GetInt32(1),Id:r.IsDBNull(2)?Guid.Empty:r.GetGuid(2)),ct)).Single();
            if(state.State=="Fenced")
                ...
                await db.ExecuteAsync(PortfolioDbSql.Financial.LegacyFinancialWriterFence.Update03,[fundId,portfolioId,retentionId],ct);
            return Convert.ToInt64(await db.ScalarAsync(PortfolioDbSql.Financial.LegacyFinancialWriterFence.Select07,[fundId],ct));
        },token);
    }
    ...
    {
        await LockScope(db,fundId,ct);
        Require(await db.ExecuteAsync(PortfolioDbSql.Financial.LegacyFinancialWriterFence.Update04,
