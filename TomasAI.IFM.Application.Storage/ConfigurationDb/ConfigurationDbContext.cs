      ...
      IConfigurationDbContext
{
    ...
    public async Task PublishAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        RequireRegimeDiscovery(kind);
        await dbFactory.ConfigurationDb
            .Use($"{nameof(ConfigurationDbSql)}.{nameof(ConfigurationDbSql.Publish)}", ConfigurationDbSql.Publish)
        ...
        CancellationToken cancellationToken = default)
    {
        RequireRegimeDiscovery(kind);
        await dbFactory.ConfigurationDb
            .Use($"{nameof(ConfigurationDbSql)}.{nameof(ConfigurationDbSql.Retire)}", ConfigurationDbSql.Retire)
    ...
    static void RequireRegimeDiscovery(StrategyParameterSetKind kind)
    {
        if (kind != StrategyParameterSetKind.RegimeDiscovery)
            throw new NotSupportedException($"The typed lifecycle for {kind} is not defined yet.");
    }
}
