...
/// <summary>
/// Handles the main, complete, and fail events in the futures tick quote-data insertion family.
/// </summary>
public static class FuturesTickQuoteDataInserted
{
    ...
    /// </summary>
    static FuturesTickQuoteDataInserted()
    {
        ServiceId = $"{LogSourceType.FuturesTickQuoteDataInserted}";
    }
    ...
    /// <summary>
    /// Projects an inserted quote event to the market-data database and publishes the matching complete
    /// or fail lifecycle event.
    /// </summary>
    /// <param name="event">The durable quote-inserted event containing the bounded quote projection payload.</param>
    ...
    public static async ValueTask<bool> ExecuteAsync(
        this FuturesTickQuoteDataInsertedEvent @event,
        ...
        ILogger<TickAggregationEventActor> logger)
    {
        ...
        var source = $"{nameof(FuturesTickQuoteDataInsertedEvent)} for EntityId: {@event.EntityId}";
        try
        {
            await dbFactory.MarketDataDb.InsertTickQuoteDataAsync(@event).ConfigureAwait(false);
        ...
        catch (Exception exception)
        {
            logger.LogErrorEvent(
                ServiceId,
                exception,
                "{Source}: quote-data projection failed",
                source);
    ...
    /// <summary>
    /// Records successful completion of a futures tick quote-data projection.
    /// </summary>
    /// <param name="event">The terminal event describing the completed quote projection.</param>
    ...
    public static ValueTask<bool> ExecuteAsync(
        this FuturesTickQuoteDataInsertedCompleteEvent @event,
        IEventActorContext context,
        ILogger<TickAggregationEventActor> logger)
    {
        IsArgumentNull.Check(@event);
    ...
    /// <summary>
    /// Records failure of a futures tick quote-data projection.
    /// </summary>
    /// <param name="event">The terminal event describing the failed quote projection.</param>
    /// <param name="context">The event actor context associated with lifecycle processing.</param>
    ...
    public static ValueTask<bool> ExecuteAsync(
        this FuturesTickQuoteDataInsertedFailEvent @event,
        IEventActorContext context,
        ILogger<TickAggregationEventActor> logger)
    {
        IsArgumentNull.Check(@event);
