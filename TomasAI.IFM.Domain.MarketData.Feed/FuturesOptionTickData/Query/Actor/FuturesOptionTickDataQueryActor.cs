    ...
    : BaseQueryActor<FuturesOptionTickDataQueryActor>(logger, new ActorMailboxId(ActorType.Query, ActorName))
{
    ...
    {
        [typeof(GetLastFuturesOptionTickDataQuery).Name] = (ctx, dbFactory, q) =>
        {
            var query = (q as GetLastFuturesOptionTickDataQuery)!;
            return query.GetLastFuturesOptionTickDataAsync(ctx, dbFactory);
        }
    };
    ...
    protected override async ValueTask OnExceptionAsync(IQueryActorContext context, ActorThreadId threadId, IQuery query, string verb, Exception ex)
    {
        ...
        try
        {
            ...
            {
                _ when query is GetLastFuturesOptionTickDataQuery
                    => context.ReplyAsync(threadId, verb, new ServiceResult<FuturesOptionTickDataV2ReadModel?>(query.ErrorCode, ex!.Message)),
