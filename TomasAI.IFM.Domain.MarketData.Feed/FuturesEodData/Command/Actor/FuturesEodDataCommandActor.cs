...
using TomasAI.IFM.Domain.MarketData.Feed.FuturesEodData.Command.Validation;
...
/// <summary>
/// Represents an actor responsible for managing futures EOD data commands and state within an event-sourced system.
/// </summary>
/// <remarks>The <see cref="FuturesEodDataCommandActor"/> is a specialized command actor designed to handle operations
/// related to futures end-of-day data. It processes commands such as inserting futures EOD data and VIX futures EOD data.
/// It validates the commands, and manages the actor's state. This actor relies on an event-sourced repository for state
...
public class FuturesEodDataCommandActor(
    ICommandActorContext<FuturesEodDataCommandActor> actorContext,
    IEventProjector<FuturesEodDataCommandActor> eventProjector)
    : BaseEventSourceCommandActor<FuturesEodDataCommandActor>(actorContext, actorContext.Logger)
{
    ...
    protected override async ValueTask OnStartup(ICommandActorContext<FuturesEodDataCommandActor> context)
    {
        IsArgumentNull.Check(context);
        _repo = IsArgumentNull.Set(context.Container.Resolve<IEventSourceActorStateRepository<FuturesEodDataCommandState>>());
        await _eventProjector.StartAsync(context).ConfigureAwait(false);
    }
    protected override async ValueTask OnShutdown(ICommandActorContext<FuturesEodDataCommandActor> context)
    ...
    /// <param name="state">The current state of the actor. Must be a valid instance of FuturesEodDataCommandState. Cannot be null.</param>
    ...
    protected override ValueTask<ServiceResult<GuidResult>> ReceiveAsync(ICommandActorContext<FuturesEodDataCommandActor> context, IActorState state, ICommand cmd)
    {
        IsArgumentNull.Check(context);
        ...
        _ = receiveFunc.Invoke(cmd, context, eodDataState);
    ...
    /// </summary>
    static readonly Dictionary<string, Func<ICommand, ICommandActorContext, FuturesEodDataCommandState, bool>> _receiveMap = new()
    {
    ...
    protected override async ValueTask OnValidateAsync(ICommandActorContext<FuturesEodDataCommandActor> context, ActorThreadId threadId, ICommand cmd)
    {
        IsArgumentNull.Check(context);
    ...
    protected override async ValueTask<IActorState> OnLoadStateAsync(ICommandActorContext<FuturesEodDataCommandActor> context, ActorThreadId threadId, ICommand cmd)
    {
        IsArgumentNull.Check(context);
        IsArgumentNull.Check(threadId);
        IsArgumentNull.Check(cmd);
    ...
    /// <summary>
    /// Asynchronously saves the current state of the futures EOD data actor in response to a command.
    /// </summary>
    ...
    /// cref="FuturesEodDataCommandState"/>.</param>
    ...
    protected override async ValueTask OnSaveStateAsync(ICommandActorContext<FuturesEodDataCommandActor> context, ActorThreadId threadId, IActorState state, ICommand cmd)
    {
        IsArgumentNull.Check(context);
        IsArgumentNull.Check(threadId);
        IsArgumentNull.Check(state);
        IsArgumentNull.Check(cmd);
        var eodDataState = IsArgumentNull.Set((state as FuturesEodDataCommandState)!);
        await _repo.SaveStateAsync(context, eodDataState, cmd);
    }
    ...
    protected override async ValueTask<ServiceResult<GuidResult>> OnExceptionAsync(ICommandActorContext<FuturesEodDataCommandActor> context, ActorThreadId threadId, ICommand command, Exception ex)
    {
        try
        {
            IsArgumentNull.Check(context);
            IsArgumentNull.Check(threadId);
            IsArgumentNull.Check(command);
            IErrorEvent<FuturesEodDataId> errorEvent = ex switch
            {
                InsertFuturesEodDataException
                    => await ex.SendErrorEventAsync<FuturesEodDataInsertedFailEvent, FuturesEodDataId>(context, (command as InsertFuturesEodDataCommand)!, FuturesEodDataInsertedEvent.Actor, FuturesEodDataInsertedEvent.Verb),
                InsertVixFuturesEodDataException
                    => await ex.SendErrorEventAsync<VixFuturesEodDataInsertedFailEvent, FuturesEodDataId>(context, (command as InsertVixFuturesEodDataCommand)!, VixFuturesEodDataInsertedEvent.Actor, VixFuturesEodDataInsertedEvent.Verb),
                _ => default!
            };
