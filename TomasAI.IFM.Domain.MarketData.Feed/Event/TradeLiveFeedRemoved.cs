        ...
        MarketDataFeedEventParameters p)
    {
        ...
        try
        {
            ...
            if (optionTrade.IsValid)
            {
                ...
                {
                    await eventApi.SendTradeLiveFeedRemovedFailEventAsync(e, new InvalidOperationException($"Trade Live Feed already inactive for OrderId: {e.OrderId}, TradeId: {e.TradeId}"));
                    return false;
                }
                foreach (var optionLeg in optionTrade.OptionLegs!)
                {
                    ...
                    await commandApi.StopFuturesOptionTickDataStreamingAsync(e.CommandId, entityId, optionLeg.ContractId);
            ...
            {
                await eventApi.SendTradeLiveFeedRemovedFailEventAsync(e, new InvalidOperationException($"Trade Live Feed does not exist for OrderId: {e.OrderId}, TradeId: {e.TradeId}"));
                return false;
            }
        }
        catch (Exception ex)
        {
            await eventApi.SendTradeLiveFeedRemovedFailEventAsync(e, ex);
