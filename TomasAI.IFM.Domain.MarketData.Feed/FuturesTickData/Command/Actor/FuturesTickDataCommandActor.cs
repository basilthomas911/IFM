...
using TomasAI.IFM.Domain.MarketData.Feed.FuturesTickData.Command.Validation;
...
/// <summary>
/// Represents an actor responsible for managing futures tick data commands and state within an event-sourced system.
/// </summary>
/// <remarks>The <see cref="FuturesTickDataCommandActor"/> is a specialized command actor designed to handle operations
/// related to futures tick data. It processes commands such as inserting tick data, starting streaming, and stopping streaming
/// of futures tick data. It validates the commands, and manages the actor's state. This actor relies on an event-sourced
...
public class FuturesTickDataCommandActor(
    ICommandActorContext<FuturesTickDataCommandActor> actorContext,
    IEventProjector<FuturesTickDataCommandActor> eventProjector)
    : BaseEventSourceCommandActor<FuturesTickDataCommandActor>(actorContext, actorContext.Logger)
{
    public const string ActorName = "FuturesTickDataCommand";
    readonly ILogger<FuturesTickDataCommandActor> _logger = IsArgumentNull.Set(actorContext.Logger);
    readonly CommandAuditTracker _commandAudit = new(IsArgumentNull.Set(actorContext.DbEventSource));
    readonly IEventProjector<FuturesTickDataCommandActor> _eventProjector = IsArgumentNull.Set(eventProjector);
    IEventSourceActorStateRepository<FuturesTickDataCommandState> _repo = default!;
    ...
    protected override async ValueTask OnStartup(ICommandActorContext<FuturesTickDataCommandActor> context)
    {
        IsArgumentNull.Check(context);
        _repo = IsArgumentNull.Set(context.Container.Resolve<IEventSourceActorStateRepository<FuturesTickDataCommandState>>());
        await _eventProjector.StartAsync(context).ConfigureAwait(false);
    }
    protected override async ValueTask OnShutdown(ICommandActorContext<FuturesTickDataCommandActor> context)
    ...
    protected override ICommand ParseMessage(ICommandActorContext<FuturesTickDataCommandActor> context, IActorMessage message)
    {
        IsArgumentNull.Check(context);
    ...
    {
        [InsertFuturesTickDataCommand.Verb] = msg => msg.AsCommand<InsertFuturesTickDataCommand>()!,
        [StartFuturesTickDataStreamingCommand.Verb] = msg => msg.AsCommand<StartFuturesTickDataStreamingCommand>()!,
        [StopFuturesTickDataStreamingCommand.Verb] = msg => msg.AsCommand<StopFuturesTickDataStreamingCommand>()!
    };
    ...
    /// <param name="state">The current state of the actor. Must be a valid instance of FuturesTickDataCommandState. Cannot be null.</param>
    ...
    protected override ValueTask<ServiceResult<GuidResult>> ReceiveAsync(ICommandActorContext<FuturesTickDataCommandActor> context, IActorState state, ICommand cmd)
    {
        IsArgumentNull.Check(context);
        IsArgumentNull.Check(state);
        IsArgumentNull.Check(cmd);
        var futuresTickDataState = IsArgumentNull.Set((state as FuturesTickDataCommandState)!);
    ...
    static readonly Dictionary<string, Func<ICommand, ICommandActorContext, FuturesTickDataCommandState, bool>> _receiveMap = new()
    {
        ...
        [typeof(StopFuturesTickDataStreamingCommand).Name] = (cmd, context, state) => (cmd as StopFuturesTickDataStreamingCommand)!.Execute(state)
    };
    ...
    protected override async ValueTask OnValidateAsync(ICommandActorContext<FuturesTickDataCommandActor> context, ActorThreadId threadId, ICommand cmd)
    {
        IsArgumentNull.Check(context);
    ...
    {
        [typeof(InsertFuturesTickDataCommand).Name] = cmd => {
        ...
        [typeof(StartFuturesTickDataStreamingCommand).Name] = cmd => {
        ...
        [typeof(StopFuturesTickDataStreamingCommand).Name] = cmd => {
    ...
    protected override async ValueTask<IActorState> OnLoadStateAsync(ICommandActorContext<FuturesTickDataCommandActor> context, ActorThreadId threadId, ICommand cmd)
    {
        IsArgumentNull.Check(context);
        IsArgumentNull.Check(threadId);
        IsArgumentNull.Check(cmd);
    ...
    /// <summary>
    /// Asynchronously saves the current state of the futures tick data actor in response to a command.
    /// </summary>
    ...
    /// cref="FuturesTickDataCommandState"/>.</param>
    ...
    protected override async ValueTask OnSaveStateAsync(ICommandActorContext<FuturesTickDataCommandActor> context, ActorThreadId threadId, IActorState state, ICommand cmd)
    {
        IsArgumentNull.Check(context);
        IsArgumentNull.Check(threadId);
        IsArgumentNull.Check(state);
        IsArgumentNull.Check(cmd);
        var futuresTickDataState = IsArgumentNull.Set((state as FuturesTickDataCommandState)!);
        await _repo.SaveStateAsync(context, futuresTickDataState, cmd);
    }
    ...
    protected override async ValueTask<ServiceResult<GuidResult>> OnExceptionAsync(ICommandActorContext<FuturesTickDataCommandActor> context, ActorThreadId threadId, ICommand command, Exception ex)
    {
        try
        {
            IsArgumentNull.Check(context);
