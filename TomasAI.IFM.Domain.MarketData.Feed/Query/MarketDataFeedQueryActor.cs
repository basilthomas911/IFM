    ...
    : BaseQueryActor<MarketDataFeedQueryActor>(logger, new ActorMailboxId(ActorType.Query, ActorName))
{
    ...
    /// <param name="state">The current state of the actor, which must be of type <see cref="Command.State.MarketDataFeedQueryState"/>.</param>
    ...
    protected override async ValueTask ReceiveAsync(IQueryActorContext context, IActorState state, IQuery query)
    {
        ...
        var resultTask = _receiveMap.ContainsKey(qryName)
            ? _receiveMap[qryName](context, _qryParameters, query)
