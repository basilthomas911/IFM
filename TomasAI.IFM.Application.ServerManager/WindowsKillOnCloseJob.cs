...
public sealed class WindowsKillOnCloseJob : IDisposable
{
    ...
    public WindowsKillOnCloseJob()
    {
        ...
        {
            throw new PlatformNotSupportedException("Development process ownership requires Windows Job Objects.");
        }

        _handle = NativeMethods.CreateJobObject(IntPtr.Zero, null);
        if (_handle.IsInvalid)
        {
            throw new Win32Exception(Marshal.GetLastWin32Error(), "Could not create the Development process Job Object.");
        }
        ...
        try
        {
            ...
            if (!NativeMethods.SetInformationJobObject(_handle, 9, pointer, (uint)length))
            {
                throw new Win32Exception(
                    ...
                    "Could not configure the Development process Job Object.");
            }
        }
        finally
        {
    ...
    public void Assign(Process process)
    {
        ...
        if (!NativeMethods.AssignProcessToJobObject(_handle, process.Handle))
        {
            throw new Win32Exception(
                Marshal.GetLastWin32Error(),
                $"Could not assign Development process {process.Id} to its Job Object.");
        }
    }
