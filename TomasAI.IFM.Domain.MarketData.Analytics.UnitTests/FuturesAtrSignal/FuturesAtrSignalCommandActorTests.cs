...
public class FuturesAtrSignalCommandActorTests : IClassFixture<MarketDataAnalyticsTestFixture>
{
    ...
    {
        public ICommand InvokeParseMessage(ICommandActorContext<FuturesAtrSignalCommandActor> context, NatsMsg<byte[]> message)
            => ParseMessage(context, message);

        public async ValueTask<ServiceResult<GuidResult>> InvokeReceiveAsync(ICommandActorContext<FuturesAtrSignalCommandActor> context, IActorState state, ICommand cmd)
            => await ReceiveAsync(context, state, cmd);

        public async ValueTask InvokeOnValidateAsync(ICommandActorContext<FuturesAtrSignalCommandActor> context, ActorThreadId threadId, ICommand cmd)
            => await OnValidateAsync(context, threadId, cmd);

        public async ValueTask InvokeOnStartup(ICommandActorContext<FuturesAtrSignalCommandActor> context)
            => await OnStartup(context);

        public async ValueTask<IActorState> InvokeOnLoadStateAsync(ICommandActorContext<FuturesAtrSignalCommandActor> context, ActorThreadId threadId, ICommand cmd)
            => await OnLoadStateAsync(context, threadId, cmd);

        public async ValueTask InvokeOnSaveStateAsync(ICommandActorContext<FuturesAtrSignalCommandActor> context, ActorThreadId threadId, IActorState state, ICommand cmd)
            => await OnSaveStateAsync(context, threadId, state, cmd);

        public async ValueTask<ServiceResult<GuidResult>> InvokeOnExceptionAsync(ICommandActorContext<FuturesAtrSignalCommandActor> context, ActorThreadId threadId, ICommand cmd, Exception ex)
    ...
    [Fact]
    public async Task ParseMessage_DeserializesGenerateFuturesAtrSignalCommand_WithoutDomainAuditWrite()
    {
        // Arrange
    ...
    [Fact]
    public async Task ParseMessage_DeserializesGenerateFuturesAtrDailySignalCommand_WithoutDomainAuditWrite()
    {
        // Arrange
        var dbEventSource = Substitute.For<IEventSourceActorDbContext>();
        var logger = Substitute.For<ILogger<FuturesAtrSignalCommandActor>>();
    ...
    public async Task OnSaveStateAsync_SavesState_ToRepository()
    {
        ...
        var repo = Substitute.For<IEventSourceActorStateRepository<FuturesAtrSignalCommandState>>();
        repo.SaveStateAsync(Arg.Any<ICommandActorContext<FuturesAtrSignalCommandActor>>(), Arg.Any<FuturesAtrSignalCommandState>(), Arg.Any<ICommand>())
