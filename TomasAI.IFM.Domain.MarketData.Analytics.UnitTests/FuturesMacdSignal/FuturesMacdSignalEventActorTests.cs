...
public class FuturesMacdSignalEventActorTests : IClassFixture<MarketDataAnalyticsTestFixture>
{
    ...
    {
        public TestableFuturesMacdSignalEventActor(IActorSupervisor supervisor, IStatusConsoleWriter statusConsoleWriter,
            ILogger<FuturesMacdSignalEventActor> logger, IMarketDataApi marketDataApi)
            : base(new FuturesMacdSignalEventContext(supervisor, statusConsoleWriter, logger, marketDataApi))
        {
        }

        public IEvent InvokeParseMessage(IEventActorContext<FuturesMacdSignalEventActor> context, NatsMsg<byte[]> message)
            => ParseMessage(context, message);

        public async ValueTask InvokeReceiveAsync(IEventActorContext<FuturesMacdSignalEventActor> context, IEvent @event)
            => await ReceiveAsync(context, @event);


        public async ValueTask InvokeOnExceptionAsync(IEventActorContext<FuturesMacdSignalEventActor> context, ActorThreadId threadId, IEvent @event, Exception ex)
            => await OnExceptionAsync(context, threadId, @event, ex);

        public async ValueTask InvokeOnStartAsync(IEventActorContext<FuturesMacdSignalEventActor> context)
            => await OnStartup(context);

        public async ValueTask InvokeOnStopAsync(IEventActorContext<FuturesMacdSignalEventActor> context)
    ...
    public void ParseMessage_WithValidMacdSignalGeneratedCompleteEvent_ShouldReturnEvent(TimeFrameType timePeriod)
    {
        ...
        var mockContext = Substitute.For<IEventActorContext<FuturesMacdSignalEventActor>>();
    ...
    public async Task OnExceptionAsync_ForEachTimePeriod_SendsEventExceptionEvent(TimeFrameType timePeriod)
    {
        ...
        // Assert
        await mockContext.Received(1).SendAsync<global::TomasAI.IFM.Shared.EventModelActor.Events.EventExceptionEvent, ActorEntityId>(
            Arg.Is<global::TomasAI.IFM.Shared.EventModelActor.Events.EventExceptionEvent>(e => e.ErrorMessage == exception.Message));
    }
    ...
    public async Task OnExceptionAsync_WithNullEvent_ShouldStillSendEventExceptionEvent()
    {
        ...
        // Assert
        await mockContext.Received().SendAsync<global::TomasAI.IFM.Shared.EventModelActor.Events.EventExceptionEvent, ActorEntityId>(
            Arg.Is<global::TomasAI.IFM.Shared.EventModelActor.Events.EventExceptionEvent>(e => e.ErrorMessage.Contains("@event")));
    }
    ...
    public async Task OnExceptionAsync_WhenSendAsyncThrows_ShouldAttemptFallbackSendAndLog()
    {
        ...
        // Assert
        await act.Should().NotThrowAsync();
        await mockContext.Received(2).SendAsync<global::TomasAI.IFM.Shared.EventModelActor.Events.EventExceptionEvent, ActorEntityId>(
            Arg.Any<global::TomasAI.IFM.Shared.EventModelActor.Events.EventExceptionEvent>());
    }
