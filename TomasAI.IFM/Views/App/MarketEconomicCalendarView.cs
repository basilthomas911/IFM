...
namespace TomasAI.IFM.Views.App
{
    public partial class MarketEconomicCalendarView : UserControl, IFormControl
    {
        ...
        public void LoadView(IAppRoot appRoot)
        {
            ...
            // set updated actions...
            _viewModel.OnErrorMessage = e => this.ShowErrorMessage(e, "Market Economic Calendar View");
            _viewModel.OnModelUpdate = e => this.Post(() => OnModelUpdate(e));
            _viewModel.OnCalendarDateUpdate = (e, ec) => this.Post(() => OnCalendarDateUpdate(e, ec));
            _viewModel.OnCountryCodesLoaded = ()=> this.Post(() => OnCountryCodesLoaded());
        ...
        internal void OnModelUpdate(EconomicCalendarReadModel[] economicCalendar)
        {
            ...
            if (economicCalendar != null && economicCalendar.Length > 0)
            {
                foreach (var e in economicCalendar)
                    lstEconomicCalendar.Items.Add(new ListViewItem(new String[]
                    {
                        $"{e.EventDate:t}",
                        $"{e.CountryCode}",
                        $"{e.EventName}"
