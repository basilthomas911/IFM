...
namespace TomasAI.IFM.Views.Trade.IronCondor
{
    public partial class IronCondorTradeOrderView : UserControl, IFormControl, ITradeOrderControl
    {
        ...
        public IronCondorTradeOrderView(Control parentControl, IronCondorTradeOrderReadModel viewModel)
        {
            ...
            _viewModel.ShowIronCondorTrade = () => this.Invoke(() =>
            {
                LoadTradeActions();
                LoadOptionTypes();
                LoadOrderTypes();
            ...
            _viewModel.ShowAssetPrice = assetPrice => this.Invoke(() => ShowAssetPrice(assetPrice));
        ...
        {
            _viewModel.LoadDefaultFuturesContractDefinitions();
        }

        public DateTime MaturityDate => dtmLeg1LastTradeDate.Value;
        ...
        public void SubmitOrder(DateTime tradeDate, OrderActionType orderActionType, TradeOrderConfirmationViewModel tradeOrderConfirmation, Action<Guid> setCommmandId)
        {
        ...
        {
            dtmLeg1LastTradeDate.Value = _viewModel.MaturityDate;
            dtmLeg2LastTradeDate.Value = _viewModel.MaturityDate;
            dtmLeg3LastTradeDate.Value = _viewModel.MaturityDate;
            dtmLeg4LastTradeDate.Value = _viewModel.MaturityDate;
        ...
        void EnableIronCondorControls()
        {
            var enabled = _viewModel.FundOrderTrade.TradeState == TradeState.NewTrade;
            var controls = new Control[] { dtmLeg1LastTradeDate, dtmLeg2LastTradeDate, dtmLeg3LastTradeDate, dtmLeg4LastTradeDate,
                ddlLeg1StrikePrice, ddlLeg2StrikePrice, ddlLeg3StrikePrice, ddlLeg4StrikePrice, nudQuantity, txtAssetPrice,
                txtLeg1BidPrice, txtLeg1AskPrice, txtLeg2BidPrice, txtLeg2AskPrice, txtLeg3BidPrice, txtLeg3AskPrice, txtLeg4BidPrice, txtLeg4AskPrice,
                ddlOrderType, ddlPrice, btnBid, btnMid, btnAsk, txtFundBalance, txtRiskMargin};
            EnableControls(enabled, controls);
        }

        void ShowLiveFeedValues(FuturesEodDataViewModel futuresEodData)
        {
            ShowTradeValues();
        ...
        void ShowTradeValues()
        {
            ...
            txtLeg1MaxLossLimit.Text = $"{(spreadTrade.TradeTypeLimits.Get(_viewModel.PutSpreadTradeType)?.MaxLossLimit ?? 0.0):F4}";
            txtLeg1MinProfitLimit.Text = $"{(spreadTrade.TradeTypeLimits.Get(_viewModel.PutSpreadTradeType)?.MinProfitLimit ?? 0.0):F4}";
            txtLeg3MaxLossLimit.Text = $"{(spreadTrade.TradeTypeLimits.Get(_viewModel.CallSpreadTradeType)?.MaxLossLimit ?? 0.0):F4}";
            txtLeg3MinProfitLimit.Text = $"{(spreadTrade.TradeTypeLimits.Get(_viewModel.CallSpreadTradeType)?.MinProfitLimit ?? 0.0):F4}";
        ...
        void LoadOptionTypes()
        {
            ...
            ddlLeg4OptionType.Enabled = false;
        }
        ...
        {
            _viewModel.UpdateDaysToExpiryAction(dtmLeg1LastTradeDate.Value);
        ...
        public void OrderActionTypeChanged(OrderActionType orderActionType)
        {
            ...
            OptionLegDataReadModel GetParentOptionLegData(TradeType tradeType, OptionLegAction optionLegAction, OptionType optionType)
            {
