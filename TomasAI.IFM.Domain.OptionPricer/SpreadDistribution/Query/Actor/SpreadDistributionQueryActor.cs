    ...
    : BaseQueryActor<SpreadDistributionQueryActor>(actorContext, actorContext.Logger)
{
    ...
    protected override async ValueTask OnExceptionAsync(IQueryActorContext<SpreadDistributionQueryActor> context, ActorThreadId threadId, IQuery query, string verb, Exception ex)
    {
        IsArgumentNull.Check(context);
