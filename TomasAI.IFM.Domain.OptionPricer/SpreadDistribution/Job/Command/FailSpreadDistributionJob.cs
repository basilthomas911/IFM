...
public static class FailSpreadDistributionJob
{
    ...
    public static ServiceResult<GuidResult> Execute(this FailSpreadDistributionJobCommand e, SpreadDistributionJobCommandState state)
        => e switch
        {
            _ when !state.IsJobStatusInProgress
                => throw new SpreadDistributionJobNotInProgressException(e.EntityId),
            _ => e.UpdateResult(() => state.Update(e.CreateSpreadDistributionJobFailedEvent(), e))
        };
