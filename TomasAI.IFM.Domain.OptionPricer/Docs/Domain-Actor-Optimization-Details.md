...
3. **Command parsing synchronously blocked actor threads on audit I/O.** Both command actors used `GetAwaiter().GetResult()`. A domain-local `CommandAuditTracker` now starts the write during parsing and asynchronously joins it before validation completes.
