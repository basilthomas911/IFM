...
public sealed class ParameterAssignmentStateRepository(
    IEventSourceActorStateFactory stateFactory,
    IEventSourceActorDbContext eventSource,
    IActorService actorService,
    IEventProjector<ParameterAssignmentCommandActor> projector,
    ILogger<ParameterAssignmentStateRepository> logger)
    : BaseEventSourceActorRepository(stateFactory, eventSource, actorService, logger),
      IEventSourceActorStateRepository<ParameterAssignmentCommandState>
{
    /// <summary>Loads the complete configuration stream.</summary>
    public async ValueTask<ParameterAssignmentCommandState> LoadStateAsync(ICommand command)
        => await LoadStateAsync<ParameterAssignmentCommandState>(command, CancellationToken.None)
    ...
    public async ValueTask SaveStateAsync(ICommandActorContext context,
        ParameterAssignmentCommandState state, ICommand command)
