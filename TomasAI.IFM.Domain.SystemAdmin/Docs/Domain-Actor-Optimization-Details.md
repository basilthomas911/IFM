...
| 7 | Non-suspending startup, save, resolver, and query-dispatch wrappers used `async` state machines | Extra state-machine complexity and possible allocation | Non-suspending paths now return `ValueTask`, cached `Task`, or values directly; genuine I/O awaits remain asynchronous |
