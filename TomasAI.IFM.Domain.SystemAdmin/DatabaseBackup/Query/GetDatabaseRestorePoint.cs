...
/// <summary>Handles <see cref="GetDatabaseRestorePointQuery"/>.</summary>
public static class GetDatabaseRestorePoint
{
    /// <summary>Reads the requested Database Backup projection and sends its typed response.</summary>
    public static async ValueTask ExecuteAsync(this GetDatabaseRestorePointQuery query, ISystemAdminDbContext dbContext, IQueryActorContext<DatabaseBackupQueryActor> context, CancellationToken cancellationToken)
    {
        var result = await dbContext.GetRestorePointAsync(query, cancellationToken);
