...
/// <summary>
/// Represents an actor responsible for managing system admin commands and state within an event-sourced system.
/// </summary>
/// <remarks>The <see cref="SystemAdminCommandActor"/> is a specialized command actor designed to handle operations
/// related to system administration such as database backups. It validates commands, processes them via the
/// appropriate handlers, and manages the actor's state. This actor relies on an event-sourced repository for state
...
public class SystemAdminCommandActor(
    IEventSourceActorDbContext dbEventSource,
    ILogger<SystemAdminCommandActor> logger)
    : BaseEventSourceCommandActor<SystemAdminCommandActor>(logger, new ActorMailboxId(ActorType.Command, ActorName))
{
    ...
    {
        [BackupDatabaseCommand.Verb] = msg => msg.AsCommand<BackupDatabaseCommand>()!
    };
    ...
    /// <param name="state">The current state of the actor. Cannot be null.</param>
    ...
    /// <summary>
    /// Provides a mapping from command type names to delegate functions that execute the corresponding system admin command
    /// logic on a given state.
    /// </summary>
    static readonly Dictionary<string, Func<ICommand, ICommandActorContext, SystemAdminCommandState, bool>> _receiveMap = new()
    {
        [typeof(BackupDatabaseCommand).Name] = (cmd, context, state) => (cmd as BackupDatabaseCommand)!.Execute(state)
    };
    ...
    /// <summary>
    /// Asynchronously saves the current state of the system admin actor in response to a command.
    /// </summary>
    ...
    protected override async ValueTask OnSaveStateAsync(ICommandActorContext context, ActorThreadId threadId, IActorState state, ICommand cmd, CancellationToken cancellationToken)
        => await _repo.SaveStateAsync(context, (SystemAdminCommandState)state, cmd, cancellationToken).ConfigureAwait(false);
