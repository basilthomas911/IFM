...
/// <summary>
/// Represents a command to remove a specific order from a fund.
/// </summary>
/// <remarks>
/// MessagePack-serializable using numeric keys:
/// - Base command members occupy keys 0–5.
/// - This command’s payload (<see cref="FundOrderId" />) uses key 6.
/// The command routes to <see cref="BoundedContextName.FundBoundedContext" /> and targets the fund identified
/// by the supplied <see cref="FundOrderId" />.
/// </remarks>
[MessagePackObject(AllowPrivate = true)]
public record RemoveOrderFromFundCommand : ICommand<FundId>
{
    public const string Actor = "FundCommand";
    public const string Verb = "RemoveOrder";
    public const int ErrorId = 2012;
    ...
    /// <summary>
    /// Gets or sets the identifier of the fund order to remove.
    /// </summary>
    ...
    /// </summary>
    public RemoveOrderFromFundCommand() { }

    /// <summary>
    /// Convenience constructor for application code.
    /// </summary>
    /// <param name="fundOrderId">The fund order identifier to remove.</param>
    public RemoveOrderFromFundCommand(FundOrderId fundOrderId)
    {
        FundOrderId = fundOrderId ?? throw new ArgumentNullException(nameof(fundOrderId));
    ...
    /// <param name="routeTo">Bounded context to route to (key 5).</param>
    /// <param name="fundOrderId">Fund order identifier payload (key 6).</param>
