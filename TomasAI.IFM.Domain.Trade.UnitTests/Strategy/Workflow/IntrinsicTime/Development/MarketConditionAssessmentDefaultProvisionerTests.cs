...
public sealed class MarketConditionAssessmentDefaultProvisionerTests
{
    ...
    public async Task Ensure_publishes_three_bound_profiles_once_and_is_idempotent()
    {
        ...
        configuration.GetEffectiveRegimeDiscoveryAsync(
            ...
            .Returns(call => Task.FromResult<ResolvedRegimeDiscoveryParameterSet?>(Regime(call.ArgAt<TimeFrameType>(1), at)));
        configuration.GetEffectiveMarketConditionAssessmentAsync(
                Arg.Any<DateTime>(), "ES.Standard", "ES", Arg.Any<TimeFrameType>(), Arg.Any<CancellationToken>())
            .Returns(call => Task.FromResult(
                published.GetValueOrDefault(call.ArgAt<TimeFrameType>(3))));
        ...
        configuration.InsertMarketConditionAssessmentDraftAsync(
            ...
            .Returns(call =>
            {
                var profile = call.ArgAt<MarketConditionAssessmentParameterSet>(0);
        ...
        configuration.PublishAsync(
            ...
            .Returns(call =>
            {
                var profile = drafts[call.ArgAt<Guid>(1)];
