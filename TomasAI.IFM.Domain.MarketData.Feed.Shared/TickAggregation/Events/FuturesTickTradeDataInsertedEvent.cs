...
public sealed record FuturesTickTradeDataInsertedEvent : IEvent<TickDataEntityId>
{
    ...
    public FuturesTickTradeDataInsertedEvent() { }
    ...
    public FuturesTickTradeDataInsertedEvent(ActorSubject subject, Guid id, TickDataEntityId entityId, long eventId, Guid commandId, string aggregateId, string eventSource, DateTime receivedOn, ushort schemaVersion, TickDataId tickDataId, AssetTypeId assetTypeId, string dataset, DateOnly definitionDate, ushort publisherId, uint instrumentId, FuturesTickTradeData tradeData)
    {
        Subject = subject;
        Id = id;
        EntityId = entityId;
