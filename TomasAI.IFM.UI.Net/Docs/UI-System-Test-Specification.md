...
G0 and G1 are implemented and have accepted Development baselines. G2 is in progress: G2-001 through G2-019 have accepted Development evidence, including operator-visible market-data feed start/stop, futures/futures-option maintenance, manual yield-curve maintenance, and an FMP treasury import selected by date in the UI. Each accepted mutation has exact command/event correlation, durable query confirmation, visible UI refresh, and successful cleanup. G2-020 is the next executable step. The remaining catalog stays documented so its data, diagnostic, and automation requirements are visible before implementation.
...
G2-001 through G2-019 have accepted Development results. The prerequisite/startup decision remains recorded in [`TestResults/G2-001-007-Development-2026-08-18.md`](TestResults/G2-001-007-Development-2026-08-18.md), the feed decision is retained in [`TestResults/G2-008-009-Development-2026-08-18.md`](TestResults/G2-008-009-Development-2026-08-18.md), and securities maintenance is recorded in [`TestResults/G2-010-015-Development-2026-08-18.md`](TestResults/G2-010-015-Development-2026-08-18.md).
...
G0 and G1 are accepted, and G2-001 through G2-019 have accepted partial-gate evidence. A gate passes only when every required step is `Passed`, cleanup succeeds, and no step is `Failed`, `BlockedDependency`, `SkippedDependency`, or `NotRun`. The FMP implementation is present; the run must supply valid provider credentials or the approved deterministic adapter, usable Development infrastructure, current ES/VX contracts, and deterministic current-market data. Existing NATS transport and lower-level integration verification are prerequisites, not substitutes for desktop process evidence. G2-020 through G2-038, G3, G4, and operator confirmation remain required for Milestone A; G5 is outside the restoration milestone.
