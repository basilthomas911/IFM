...
G0 and G1 are implemented and have accepted Development baselines. G2 is in progress: G2-001 through G2-034 have accepted Development evidence, including operator-visible market-data feed start/stop, futures/futures-option maintenance, manual yield-curve, economic-calendar, and lookup maintenance, UI-selected FMP treasury and economic-calendar imports, reusable-fund cash deposit/compensation, and order/trade create, state-change, and child-first removal. Each accepted mutation has exact command/event correlation, durable query confirmation, visible UI refresh, and successful cleanup. G2-035 is the next executable step. The remaining catalog stays documented so its data, diagnostic, and automation requirements are visible before implementation.
...
| G2-009 | Stop the current market-data feed from the UI | One stop command for the started feed completes, the UI shows the inactive state, and no logical feed stream started by the command remains active |
| G2-010 | Add an isolated futures contract definition | An exact run-owned far-future contract is created through the domain command, becomes durably queryable, and appears in the UI; the fixture metadata and changed description carry the run prefix |
...
| G2-012 | Add an isolated futures option definition | An exact run-owned far-future option linked to the isolated futures contract is created, queryable, and visible; the fixture metadata and description carry the run prefix |
...
| G2-024 | Add an isolated lookup type/value | A brand-new run-prefixed lookup partition receives one order-zero value through the domain command; exact source/complete correlation, the typed partition query, and the refreshed editor agree on every business field |
| G2-025 | Change the isolated lookup type/value | The short code and description change without changing partition/order identity; exact-ID completion succeeds and durable/UI state contains only the changed value |
| G2-026 | Remove the isolated lookup type/value | The sole run-owned value is removed through the domain command, and both the typed partition query and refreshed lookup-name selector prove the isolated partition is absent without reordering operator data |
| G2-027 | Resolve the designated G2 fund fixture | A valid named non-production fund is selected in the Fund Transactions editor; if it does not exist, explicit retention approval permits one creation through the real Trade UI/domain command, and the exact source/terminal command ID, durable row, selector, and balance must agree |
| G2-028 | Create a reversible fund transaction | The Fund Transactions editor submits a positive run-referenced cash deposit through the domain command; exact source/complete correlation succeeds, the immutable row exists exactly once, and durable/displayed balances increase by the configured amount |
| G2-029 | Compensate the fund transaction | The same editor submits an equal run-referenced cash withdrawal; exact source/complete correlation succeeds, the baseline balance is restored exactly, and both append-only entries remain visible and durable; finally-path reconciliation supplies a missing withdrawal after an interrupted partial run |
...
G2-001 through G2-034 have accepted Development results. The prerequisite/startup decision remains recorded in [`TestResults/G2-001-007-Development-2026-08-18.md`](TestResults/G2-001-007-Development-2026-08-18.md), the feed decision is retained in [`TestResults/G2-008-009-Development-2026-08-18.md`](TestResults/G2-008-009-Development-2026-08-18.md), securities maintenance is recorded in [`TestResults/G2-010-015-Development-2026-08-18.md`](TestResults/G2-010-015-Development-2026-08-18.md), yield-curve maintenance/import is recorded in [`TestResults/G2-016-019-Development-2026-08-18.md`](TestResults/G2-016-019-Development-2026-08-18.md), economic-calendar maintenance/import is recorded in [`TestResults/G2-020-023-Development-2026-08-18.md`](TestResults/G2-020-023-Development-2026-08-18.md), lookup maintenance is recorded in [`TestResults/G2-024-026-Development-2026-08-18.md`](TestResults/G2-024-026-Development-2026-08-18.md), reusable-fund compensation is recorded in [`TestResults/G2-027-029-Development-2026-08-18.md`](TestResults/G2-027-029-Development-2026-08-18.md), and order/trade mutation is recorded in [`TestResults/G2-030-034-Development-2026-08-18.md`](TestResults/G2-030-034-Development-2026-08-18.md).

Run `20260818-170357-a09e681e82d84bab8fb514a427f358ef` passed the isolated 11-step G2-001 through G2-007 plus G2-016 through G2-019 slice in 2 minutes 53 seconds. Through the real Development UI it added, changed, and removed an isolated far-future yield-curve row; each operation produced a source event and successful terminal event with the exact command ID and agreed with typed durable and refreshed visible state. The operator-selected import date `2026-07-17` reached the parameter-only domain event, production `IReferenceDataApi` returned one canonical FMP row, the successful exact-ID terminal event carried that accepted provider result, and the durable row matched it. Cleanup used the public domain remove command to restore the captured empty import-date baseline, and all listeners and owned processes stopped normally. This accepts G2-016 through G2-019; it does not claim G2-020 through G2-038. The reviewed result is retained in [`TestResults/G2-016-019-Development-2026-08-18.md`](TestResults/G2-016-019-Development-2026-08-18.md).

Run `20260818-181651-ba7ec8589e974113a311ae735fd19a39` passed the isolated 11-step G2-001 through G2-007 plus G2-020 through G2-023 slice in 1 minute 41 seconds. The real Development editor added, changed, and removed one isolated far-future US economic-calendar row; each transition correlated its source and successful terminal events by exact command ID and agreed with bounded durable and refreshed UI state. The UI-selected `2026-07-17`/`US` import reached the parameter-only domain event; production FMP returned 23 canonical rows, the complete event carried the same rows, and bounded durable plus visible state each contained all 23. Cleanup removed the imported rows through public MarketData domain commands and restored the captured empty baseline. This accepts G2-020 through G2-023; it does not claim G2-024 through G2-038. The reviewed result is retained in [`TestResults/G2-020-023-Development-2026-08-18.md`](TestResults/G2-020-023-Development-2026-08-18.md).

Run `20260818-191132-8c41fd5c72034044b5eda0581c26b54d` passed the isolated 10-step G2-001 through G2-007 plus G2-024 through G2-026 slice in 1 minute 28 seconds. The real Development lookup editor created one new run-owned partition/value, changed its short code and description without changing its order-zero identity, and removed the sole value. Each transition correlated the exact source and successful terminal command ID, matched the typed durable partition query, and refreshed the visible name/short-code selectors. The empty pre-run lookup baseline was restored through public Reference domain commands, and all listeners and owned processes stopped normally. This accepts G2-024 through G2-026; it does not claim G2-027 through G2-038. The reviewed result is retained in [`TestResults/G2-024-026-Development-2026-08-18.md`](TestResults/G2-024-026-Development-2026-08-18.md).

Run `20260818-224604-7187833ac9354523bb3bcba7733129a7` passed the isolated 10-step G2-001 through G2-007 plus G2-027 through G2-029 slice in 1 minute 18 seconds. The real Development Fund Transactions editor selected retained non-production fund `2201`, submitted a `123.45` cash deposit, and then submitted an equal withdrawal. Both operations correlated `FundTransactionEvent` with `FundTransactionCreatedCompleteEvent` under their exact command IDs, appeared exactly once in typed durable and refreshed UI state, and moved the balance from `100000.00` to `100123.45` and back to `100000.00`. The finally path confirmed the restored balance without an extra mutation, and all listeners and owned processes stopped normally. This accepts G2-027 through G2-029; it does not claim G2-030 through G2-038. The reviewed result is retained in [`TestResults/G2-027-029-Development-2026-08-18.md`](TestResults/G2-027-029-Development-2026-08-18.md).

Run `20260819-025747-ab853f5ecb504d80bb54fc88b046db5c` passed the isolated 13-step G2-001 through G2-007 plus G2-027 and G2-030 through G2-034 slice in 6 minutes 33 seconds. The real Development Trade Orders editor added order `2801`, added Short Iron Condor trade `501` using the same ID displayed in the modal, changed the trade from `NewTrade` to `OrderSubmitted`, removed the child trade, and then removed its parent order. Every transition correlated its source and successful terminal event by exact command ID and agreed with typed durable and refreshed visible state. The ordinary run reference initialized a four-leg editable Iron Condor template without requiring a pre-existing option-trade row. Cleanup confirmed the captured zero-order/zero-trade baseline, all listeners and owned processes stopped normally, and the accepted logs contained no error, fatal, rejected-realtime, or Iron Condor load-error entry. This accepts G2-030 through G2-034; it does not claim G2-035 through G2-038. The reviewed result is retained in [`TestResults/G2-030-034-Development-2026-08-18.md`](TestResults/G2-030-034-Development-2026-08-18.md).
...
- Lookup storage identity is lookup name plus order ID. G2 therefore creates a brand-new partition containing one order-zero value; changing preserves that identity, and removing the sole value cannot trigger the storage reorder behavior against operator-owned lookup rows. Cleanup uses only the public Reference command API.
