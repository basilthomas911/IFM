...
G0 and G1 are implemented and have accepted Development baselines. G2 is in progress: G2-001 through G2-015 have accepted Development evidence, including operator-visible market-data feed start/stop and futures/futures-option maintenance with exact command/event correlation, durable query confirmation, visible UI refresh, and clean fixture removal. G2-016 is the next executable step. The remaining catalog stays documented so its data, diagnostic, and automation requirements are visible before implementation.
...
G2-001 through G2-015 have accepted Development results. The prerequisite/startup decision remains recorded in [`TestResults/G2-001-007-Development-2026-08-18.md`](TestResults/G2-001-007-Development-2026-08-18.md), and the feed decision is retained in [`TestResults/G2-008-009-Development-2026-08-18.md`](TestResults/G2-008-009-Development-2026-08-18.md). Run `20260818-142242-7667216be70b422a88b3bf4cc3f63c92` passed the isolated 13-step G2-001 through G2-007 plus G2-010 through G2-015 slice in 2 minutes 38 seconds. It proved futures and futures-option add/change/remove through the real UI, 12 source/complete events correlated by six exact command IDs, durable state after every operation, stored changed descriptions after explicit editor reload, child-before-parent removal, empty compensating cleanup, and normal process/listener shutdown. The slice relies on the already accepted G2-008 through G2-009 decision rather than restarting the live feed during securities maintenance. This accepts G2-010 through G2-015; it does not claim G2-016 through G2-038. The reviewed result is retained in [`TestResults/G2-010-015-Development-2026-08-18.md`](TestResults/G2-010-015-Development-2026-08-18.md).
...
G0 and G1 are accepted, and G2-001 through G2-015 have accepted partial-gate evidence. A gate passes only when every required step is `Passed`, cleanup succeeds, and no step is `Failed`, `BlockedDependency`, `SkippedDependency`, or `NotRun`. The FMP implementation is present; the run must supply valid provider credentials or the approved deterministic adapter, usable Development infrastructure, current ES/VX contracts, and deterministic current-market data. Existing NATS transport and lower-level integration verification are prerequisites, not substitutes for desktop process evidence. G2-016 through G2-038, G3, G4, and operator confirmation remain required for Milestone A; G5 is outside the restoration milestone.
