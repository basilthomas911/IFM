...
**Implementation module:** `Framework.TradeBroker.InteractiveBrokers.OrderExecution`  
...
**Companion specifications:** `OrderExecutionWorkflowSpecification.md`, `IbkrBrokerConnectionSpecification.md`, `IbkrBrokerAccountSpecification.md`  
...
This specification owns the order-execution-specific IBKR implementation:

- order-execution use of the shared TWS/IB Gateway connection;
- order-specific use of the official C# TWS API request and callback model;
- durable order-ID reservation and order-correlation state built on the shared order-ID allocator;
...
- order-specific IBKR error, warning, and operational handling;
- adapter-specific testing, diagnostics, security, and release acceptance.

`Framework.TradeBroker.InteractiveBrokers.Connection` separately owns the physical socket, `EWrapper`, reader loop, outbound dispatcher, API client ID, request/order ID namespaces, connection lifecycle, session epoch, reconnect supervision, and callback routing. The `OrderExecution` module must not create a second broker connection.
...
IBKR order execution shall be implemented in `Framework.TradeBroker.InteractiveBrokers.OrderExecution`, which implements broker-neutral ports defined by `Framework.TradeBroker` and consumes the provider's internal `Connection` module.
...
The adapter translates outbound broker-neutral commands into IBKR calls dispatched through the shared connection and translates callbacks routed by that connection into immutable broker-neutral messages.
...
flowchart TD
    ...
    P --> A["IBKR OrderExecution module"]
    A --> C["IBKR Connection module"]
    C --> T["Official C# TWS API"]
...
The `OrderExecution` module may depend on trade-broker ports/contracts, the provider's shared `Connection` module, and official `IBApi` types. Domain, application, projection, and policy assemblies depend on `Framework.TradeBroker`, not the concrete IBKR provider or official `IBApi` types.

`Framework.MarketData.Databento` remains the separate concrete implementation of `Framework.MarketData`. It supplies broker-neutral prices and canonical instrument identity to the workflow; `Framework.TradeBroker.InteractiveBrokers` must not take a direct project dependency on the Databento implementation.
...
|---|---|---|
| 1 | Shared connection integration and order callback routing | Pinned official API, validated shared connection, order callback registration, outbound serialization, readiness state |
...
- Connection through either TWS or IB Gateway, supplied by `Framework.TradeBroker.InteractiveBrokers.Connection`.
- One configured trading account per order adapter instance.
- One dedicated nonzero API client ID per running shared connection instance.
- One logical broker session epoch supplied by the shared connection.
...
## 7. Phase 1: Shared Connection Integration and Order Message Pumps
...
Framework.TradeBroker.InteractiveBrokers/
  Connection/
    ...
    Registration/
      IbkrFeatureRegistration.cs
      IbkrRequestRouteRegistry.cs
      IbkrOrderRouteRegistry.cs
      IbkrSingletonStreamLeaseManager.cs
    Connectivity/
      IbkrConnectionHealth.cs
      IbkrReconnectSupervisor.cs
      IbkrConnectionMetrics.cs

  OrderExecution/
    Configuration/
      IbkrExecutionOptions.cs
      IbkrExecutionConfigurationValidator.cs
    Contracts/
...
Connection files must not be copied into the `OrderExecution` module. Names may be adapted to the existing solution, but the provider-project and internal-module ownership boundaries are normative.
...
public sealed record IbkrExecutionOptions(
    string AccountId,
    bool EnableOrderSubmission,
    TimeSpan InitialOrderSynchronizationTimeout,
...
- The execution account ID must match the account verified by `Framework.TradeBroker.InteractiveBrokers.BrokerAccount` through the shared connection.
...
- Secrets and login credentials are not accepted through either configuration.
...
The shared connection supports both hosts through the same API surface.
...
Only one shared connection process may own the configured `(environment, host, port, clientId)` tuple. Feature implementations in that process register with it; they do not acquire independent physical-connection leases.
...
### 7.5 Connection state and order readiness
...
public enum IbkrExecutionAdapterStatus : byte
{
    Stopped = 0,
    AwaitingConnection = 1,
    SynchronizingOrders = 2,
...
`IbkrConnectionStatus.Ready` means the shared physical connection, reader, writer, ID allocators, and callback router are healthy. It does not authorize an order.

`IbkrExecutionAdapterStatus.ReadyForOrders` requires all of the following:

- socket connected;
- shared reader loop alive;
- a valid connection/session epoch;
- `nextValidId` received;
- configured account confirmed and its new-risk gate open through `Framework.TradeBroker.InteractiveBrokers.BrokerAccount`;
...
- shared outbound command pump healthy;
- shared callback router and order-specific callback channel healthy;
- order submission enabled for the configured environment;
- no global or account execution block active.

The shared connection cannot set `ReadyForOrders`; socket connection alone never authorizes execution.
...
1. `Framework.TradeBroker.InteractiveBrokers.Connection` creates the official API objects.
...
5. The single `EWrapper` bridge captures callbacks and routes them by request ID, order ID, ticker ID, or registered singleton-stream owner.
6. `Framework.TradeBroker.InteractiveBrokers.OrderExecution` receives only its routed order/execution/error callbacks and normalizes them into broker-neutral messages.
7. The shared single-writer command pump serializes outbound `EClientSocket` operations from every registered feature.
...
IBKR order callbacks are low volume relative to market data and must be lossless at the process level. The shared callback router shall route them into an order-specific channel owned by `Framework.TradeBroker.InteractiveBrokers.OrderExecution`.
...
All outbound `EClientSocket` calls from every TWS API module are serialized by one writer owned by `Framework.TradeBroker.InteractiveBrokers.Connection`.
...
- Preserve total accepted command order across order, account, contract-reference, market-data, and margin-preview features.
...
The shared connection must distinguish:
...
1. The shared connection leaves `Ready` and emits a connection-state event.
2. `Framework.TradeBroker.InteractiveBrokers.OrderExecution` leaves `ReadyForOrders` and rejects new normal order commands locally.
3. The shared connection preserves callback processing where possible and reconnects with bounded backoff.
4. The shared connection establishes a new session epoch and reacquires broker ID baselines.
5. It notifies every registered feature that current-epoch resynchronization is required.
6. The broker-account implementation resynchronizes account/positions; the order implementation synchronizes open/completed orders and executions.
7. The order workflow reconciles every affected active attempt.
8. The order adapter returns to `ReadyForOrders` only after connection readiness, account gate, order synchronization, and reconciliation all succeed.
