...
public sealed record ResolveForSelectionQuery : IQuery<PortfolioFundStrategySnapshot>
{
    ...
    public ResolveForSelectionQuery(ActorSubject subject, ActorEntityId entityId, int portfolioId, int? fundId, int tradingYear, string decisionHorizon, string underlyingRoot, DateTime asOfUtc, Guid workflowId, long workflowRevision, Guid requestCorrelationId, Guid correlationId, DateTime requestedOnUtc, PortfolioAccessContext access)
    {
        Subject = subject;
        EntityId = entityId;
        PortfolioId = portfolioId;
