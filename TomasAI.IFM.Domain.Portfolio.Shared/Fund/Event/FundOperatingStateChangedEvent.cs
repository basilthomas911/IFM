...
/// <summary>Represents the FundOperatingStateChangedEvent actor event.</summary>
[MessagePackObject(AllowPrivate = true)]
public sealed record FundOperatingStateChangedEvent : IPortfolioFundDomainEvent
{
    public const string Actor = "PortfolioEvent";
    public const string Verb = nameof(FundOperatingStateChangedEvent);
    ...
    [Key(14)] public FundOperatingState State { get; init; } = default!;
    [Key(15)] public string Reason { get; init; } = default!;

    [IgnoreMember] public string UserName => Principal;
    [IgnoreMember] public string EventName => nameof(FundOperatingStateChangedEvent);
    [IgnoreMember] public EventType EventType => EventType.DomainEvent;

    /// <summary>Initializes an empty event for serialization.</summary>
    public FundOperatingStateChangedEvent() { }
    ...
    public FundOperatingStateChangedEvent(Guid id, Guid commandId, long revision, DateTime occurredOnUtc, string principal, FundOperatingState state, string reason)
    {
        Id = id;
        CommandId = commandId;
        Revision = revision;
