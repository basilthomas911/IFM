...
public sealed record RecordFundOrderRiskOutcomeCommand : ICommand<PortfolioFundId>, ICommandRetryIdentity
{
    ...
    public RecordFundOrderRiskOutcomeCommand(Guid commandId, ActorSubject subject, bool postEvents, PortfolioFundId entityId, int errorCode, BoundedContextName routeTo, PortfolioFundOrderId orderId, long expectedVersion, RiskManagementResultReference result, Guid correlationId, DateTime requestedOnUtc, PortfolioAccessContext access)
    {
        CommandId = commandId;
        Subject = subject;
