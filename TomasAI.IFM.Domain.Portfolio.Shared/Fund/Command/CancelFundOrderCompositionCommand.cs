...
public sealed record CancelFundOrderCompositionCommand : ICommand<PortfolioFundId>, ICommandRetryIdentity
{
    ...
    public CancelFundOrderCompositionCommand(Guid commandId, ActorSubject subject, bool postEvents, PortfolioFundId entityId, int errorCode, BoundedContextName routeTo, PortfolioFundOrderId orderId, long expectedVersion, string reason, Guid correlationId, DateTime requestedOnUtc, PortfolioAccessContext access)
    {
        CommandId = commandId;
        Subject = subject;
