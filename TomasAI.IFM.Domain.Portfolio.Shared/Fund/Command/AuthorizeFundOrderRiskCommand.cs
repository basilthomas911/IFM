...
public sealed record AuthorizeFundOrderRiskCommand : ICommand<PortfolioFundId>, ICommandRetryIdentity
{
    ...
    /// <param name="authorization">The Authorization field.</param>
    ...
    public AuthorizeFundOrderRiskCommand(Guid commandId, ActorSubject subject, bool postEvents, PortfolioFundId entityId, int errorCode, BoundedContextName routeTo, PortfolioFundOrderId orderId, long expectedVersion, Financial.FundRiskAuthorizationReference authorization, Guid correlationId, DateTime requestedOnUtc, PortfolioAccessContext access)
    {
        CommandId = commandId;
        Subject = subject;
