...
/// <summary>Represents the DelegateFundRiskEnvelopeCommand actor message.</summary>
[MessagePackObject(AllowPrivate = true)]
public sealed record DelegateFundRiskEnvelopeCommand : ICommand<PortfolioId>, ICommandRetryIdentity
{
    ...
    /// <param name="envelope">The Envelope field.</param>
    ...
    public DelegateFundRiskEnvelopeCommand(Guid commandId, ActorSubject subject, bool postEvents, PortfolioId entityId, int errorCode, BoundedContextName routeTo, FundRiskEnvelopeReadModel envelope, long expectedPortfolioVersion, Guid correlationId, DateTime requestedOnUtc, PortfolioAccessContext access)
    {
        CommandId = commandId;
        Subject = subject;
    ...
    public const string Verb = "DelegateFundRiskEnvelope";
    ...
    [Key(6)] public FundRiskEnvelopeReadModel Envelope { get; init; } = default!;
    ...
    [IgnoreMember] public string CommandName => nameof(DelegateFundRiskEnvelopeCommand);
    ...
    public DelegateFundRiskEnvelopeCommand() { }

    /// <summary>Initializes the command from its application values.</summary>
    /// <param name="envelope">The Envelope command value.</param>
    /// <param name="expectedPortfolioVersion">The ExpectedPortfolioVersion command value.</param>
    public DelegateFundRiskEnvelopeCommand(FundRiskEnvelopeReadModel envelope, long expectedPortfolioVersion)
    {
        Envelope = envelope;
        ExpectedPortfolioVersion = expectedPortfolioVersion;
    }
