...
/// <summary>Represents the PortfolioCreatedEvent actor event.</summary>
[MessagePackObject(AllowPrivate = true)]
public sealed record PortfolioCreatedEvent : IPortfolioDomainEvent
{
    public const string Actor = "PortfolioEvent";
    public const string Verb = nameof(PortfolioCreatedEvent);
    ...
    [Key(14)] public PortfolioReadModel Portfolio { get; init; } = default!;
    [Key(15)] public Guid IdempotencyKey { get; init; } = default!;

    [IgnoreMember] public string UserName => Principal;
    [IgnoreMember] public string EventName => nameof(PortfolioCreatedEvent);
    ...
    public PortfolioCreatedEvent(Guid id, Guid commandId, long revision, DateTime occurredOnUtc, string principal, PortfolioReadModel portfolio, Guid idempotencyKey = default)
    {
        Id = id;
        CommandId = commandId;
        Revision = revision;
