...
/// <summary>Represents the PortfolioFinancialPolicyAssignedEvent actor event.</summary>
[MessagePackObject(AllowPrivate = true)]
public sealed record PortfolioFinancialPolicyAssignedEvent : IPortfolioDomainEvent
{
    public const string Actor = "PortfolioEvent";
    public const string Verb = nameof(PortfolioFinancialPolicyAssignedEvent);
    ...
    [Key(14)] public int PolicyId { get; init; } = default!;
    [Key(15)] public long PolicyVersion { get; init; } = default!;

    [IgnoreMember] public string UserName => Principal;
    [IgnoreMember] public string EventName => nameof(PortfolioFinancialPolicyAssignedEvent);
    [IgnoreMember] public EventType EventType => EventType.DomainEvent;

    /// <summary>Initializes an empty event for serialization.</summary>
    public PortfolioFinancialPolicyAssignedEvent() { }
    ...
    /// <param name="policyId">The policyId value.</param>
    /// <param name="policyVersion">The policyVersion value.</param>
    public PortfolioFinancialPolicyAssignedEvent(Guid id, Guid commandId, long revision, DateTime occurredOnUtc, string principal, int policyId, long policyVersion)
    {
        Id = id;
        CommandId = commandId;
        Revision = revision;
