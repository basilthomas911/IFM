...
public sealed record FundMandateReadModel
{
    ...
    // MessagePack keys are permanent once published. These legacy values are no longer used by the
    // domain, but must remain readable so persisted FundMandateCreatedEvent events can still be replayed.
    [Obsolete("Retained for MessagePack wire compatibility. Do not use in new business logic.")]
