...
public sealed record ConfigureLedgerCommand : ICommand<LedgerPortfolioId>, IFinancialRequest<LedgerConfigurationRequest>
{
    ...
    public ConfigureLedgerCommand(int schemaVersion, Guid commandId, ActorSubject subject, bool postEvents, LedgerPortfolioId entityId, int errorCode, BoundedContextName routeTo, Guid operationId, int portfolioId, Guid correlationId, Guid causationId, DateTime requestedAtUtc, DateTime expiresAtUtc, long expectedFinancialRevision, LedgerConfigurationRequest body, string inputSha256, FinancialAccess access)
    {
    ...
    public const string Verb = "Configure";
    ...
    [Key(5)] public int ErrorCode { get; init; } = 34125;
    ...
    [Key(14)] public LedgerConfigurationRequest Body { get; init; } = new();
    [Key(15)] public string InputSha256 { get; init; } = string.Empty;
    [Key(16)] public FinancialAccess Access { get; init; } = new(string.Empty, []);
    [IgnoreMember] public string CommandName => nameof(ConfigureLedgerCommand);
