...
public sealed record CreatePortfolioFinancialPolicyCommand : ICommand<PortfolioFinancialPolicyId>, ICommandRetryIdentity
{
    ...
    public CreatePortfolioFinancialPolicyCommand(Guid commandId, ActorSubject subject, bool postEvents, PortfolioFinancialPolicyId entityId, int errorCode, BoundedContextName routeTo, PortfolioFinancialPolicyReadModel policy, Guid idempotencyKey, Guid correlationId, DateTime requestedOnUtc, PortfolioAccessContext access)
    {
        CommandId = commandId;
        Subject = subject;
