...
The API host awaits actor startup without `Task.Run(...).Wait()` and invokes the supervisor's idempotent shutdown after HTTP intake stops. Production and integration hosts now share `ActorRuntimeStartup`, which includes actor/producer/consumer registration inside the same cancellation and rollback boundary as runtime startup. Cancellation during partial registration or startup triggers non-cancelable supervisor cleanup before the exception returns. The supervisor stops message consumers, drains accepted mailbox work, and stops actor-owned producers. See `Docs/Solution-Wide-Graceful-Cancellation-Implementation-Details.md` for the complete semantics, implemented coverage, and exclusions.

The shared runtime also publishes startup/shutdown duration and outcome counters, canceled lifecycle waits, cleanup failures by bounded stage, and the number of accepted mailbox messages completed during shutdown drain. These use the `TomasAI.IFM.Shared.EventModelActor` meter. Drain counting remains inactive during normal operation and introduces no per-message allocation.
