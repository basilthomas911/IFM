...
/// <summary>Provides legacy MDI forward-loss thresholds for option trade strategies.</summary>
public static class FuturesMDIMap
{
    static readonly Dictionary<FuturesMDIId, FuturesMDIItem> Items = new()
    {
        [new(TradeType.ShortIronCondor, 100)] = new(TradeType.ShortIronCondor, 100, 0.80),
    ...
    public static FuturesMDIItem Get(TradeType tradeType, double mdi) => Get(tradeType, mdi switch
    {
        >= 100 => 100,
        >= 90 => 90,
        >= 80 => 80,
    ...
    static FuturesMDIItem Get(TradeType tradeType, int mdi)
        => Items.GetValueOrDefault(new FuturesMDIId(tradeType, mdi));
}
