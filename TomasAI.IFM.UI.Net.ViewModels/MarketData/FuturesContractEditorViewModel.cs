    ...
    : BaseEditorViewModel
{
    ...
    public Task AddFuturesContract(FuturesContractV2ReadModel futuresContract, bool overwrite)
    ...
    public Task ChangeFuturesContract(FuturesContractId originalContractId, FuturesContractV2ReadModel futuresContract, bool overwrite, Action<bool> changeCompleted)
        =>  _commandModel.ExecuteAsync(async model => {
            model.OnError((errorCode, errorMsg) => OnError(errorCode, errorMsg));
    ...
    public Task RemoveFuturesContract(FuturesContractId contractId, bool overwrite)
        =>  _commandModel.ExecuteAsync(async model => {
                model.OnError((errorCode, errorMsg) => OnError(errorCode, errorMsg));
    ...
    /// loaded.</param>
    public Task LoadFuturesContractIds(string? contractId = default)
    ...
    public Task LoadFuturesContracts()
        => _queryModel.ExecuteAsync(async model =>  {
            await model.GetFuturesContractsAsync(futuresContracts =>
            {
                _futuresContracts = [];
    ...
    Task RefreshAddedFuturesContractIds(string consoleStatus)
        => _queryModel.ExecuteAsync(async model =>  {
            await model.GetFuturesContractsAsync( futuresContracts => {
    ...
    /// </summary>
    /// <param name="consoleStatus"></param>
    Task RefreshRemovedFuturesContractIds( string consoleStatus)
        => _queryModel.ExecuteAsync(async model => {
            await model.GetFuturesContractsAsync(futuresContracts => {
                UpdateFuturesContracts(futuresContracts);
    ...
    /// </summary>
    /// <param name="consoleStatus"></param>
    Task RefreshChangedFuturesContractIds(string consoleStatus)
        => _queryModel.ExecuteAsync(async model =>  {
            await model.GetFuturesContractsAsync(futuresContracts => {
                UpdateFuturesContracts(futuresContracts);
