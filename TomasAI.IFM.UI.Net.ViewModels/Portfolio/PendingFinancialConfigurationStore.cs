...
public sealed record PendingFinancialConfiguration(ConfigureLedgerCommand Request,PendingFinancialPhase Phase,
    string Message,LedgerConfigurationCompletedEvent? Completion=null);

public interface IPendingFinancialConfigurationStore
{
    Task<PendingFinancialConfiguration?> LoadAsync(Guid operationId,CancellationToken token=default);
    Task<PendingFinancialConfiguration> SaveAsync(PendingFinancialConfiguration operation,CancellationToken token=default);
    Task<IReadOnlyList<PendingFinancialConfiguration>> ListAsync(int portfolioId,CancellationToken token=default);
}
...
public sealed class PendingFinancialConfigurationStore(string directory):IPendingFinancialConfigurationStore
{
    readonly string _directory=Path.GetFullPath(directory);
    public static PendingFinancialConfigurationStore ForCurrentUser()=>new(Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"IFM","Portfolio","PendingFinancialConfigurations"));

    public async Task<PendingFinancialConfiguration?> LoadAsync(Guid operationId,CancellationToken token=default)
    {
        ...
        var result=JsonSerializer.Deserialize<PendingFinancialConfiguration>(await File.ReadAllTextAsync(path,token))
    ...
    public async Task<PendingFinancialConfiguration> SaveAsync(PendingFinancialConfiguration operation,CancellationToken token=default)
    {
    ...
    public async Task<IReadOnlyList<PendingFinancialConfiguration>> ListAsync(int portfolioId,CancellationToken token=default)
    {
        if(!Directory.Exists(_directory)) return [];
        var result=new List<PendingFinancialConfiguration>();
        foreach(var file in Directory.EnumerateFiles(_directory,"*.json"))
        {
            ...
            if(item?.Request.PortfolioId==portfolioId)
                result.Add(item);
        }
    ...
    static void Validate(PendingFinancialConfiguration value)
    {
