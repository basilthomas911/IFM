...
public interface IParameterSetsApi
{
 ...
 Task<ServiceResult<ParameterSchemaDefinition>> SchemaAsync(int schemaVersion,CancellationToken token=default,string componentCode=ParameterSchemaRegistry.RegimeComponent);
 ...
 Task<ServiceResult<string>> PreviewAsync(Guid setId,CancellationToken token=default,int targetHorizon=(int)TomasAI.IFM.Domain.MarketData.Analytics.Shared.TimeFrameType.Daily,string? sourcePayloadJson=null,bool rebuildIntervals=false,string componentCode=ParameterSchemaRegistry.RegimeComponent);
 Task<ServiceResult<ParameterValidationReport>> ValidateAsync(string json,int schemaVersion,CancellationToken token=default,string componentCode=ParameterSchemaRegistry.RegimeComponent);
