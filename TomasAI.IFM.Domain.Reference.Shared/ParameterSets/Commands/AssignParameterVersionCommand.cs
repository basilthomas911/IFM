...
public sealed record AssignParameterVersionCommand:ICommand<ParameterAssignmentEntityId>
{

    /// <summary>Creates an empty command for serialization.</summary>
    public AssignParameterVersionCommand() { }
    ...
    public AssignParameterVersionCommand(Guid commandId, ActorSubject subject, bool postEvents, ParameterAssignmentEntityId entityId, int errorCode, BoundedContextName routeTo, long expectedRevision, ParameterAssignmentScope scope, ParameterVersionRef reference)
    {
        CommandId = commandId;
        Subject = subject;
