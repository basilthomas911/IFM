...
public static class GetFundOrderTradeByTradeId
{
    ...
    public static ValueTask ExecuteAsync<TActor>(this GetFundOrderTradeByTradeIdQuery query, IQueryActorContext<TActor> context, PortfolioQueryParameters parameters, CancellationToken cancellationToken)
        where TActor : IActor
        => PortfolioQueryHandlerModel.ReplyAsync(context, query, parameters.Service.GetTradeAsync(query.TradeId, cancellationToken));
}
