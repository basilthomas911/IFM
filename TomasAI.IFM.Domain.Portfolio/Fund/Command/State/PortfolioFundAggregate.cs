...
public sealed class PortfolioFundAggregate
{
    ...
    public IPortfolioFundDomainEvent Create(Guid commandId, FundMandateReadModel mandate, DateTime nowUtc, string principal)
    {
        ...
        return ApplyAndReturn(new FundMandateCreatedEvent(Guid.NewGuid(), commandId, Revision + 1, nowUtc, principal, mandate.DefensiveCopy()));
    }
        ...
        string principal)
    {
        ...
        return ApplyAndReturn(new FundMandateVersionAddedEvent(Guid.NewGuid(), commandId, Revision + 1, nowUtc, principal, replacement.DefensiveCopy()));
    }
        ...
        string principal)
    {
        ...
        return ApplyAndReturn(new FundOperatingStateChangedEvent(Guid.NewGuid(), commandId, Revision + 1, nowUtc, principal, state, reason.Trim()));
    }
        ...
        string principal)
    {
        ...
        return ApplyAndReturn(new FundTradeTemplateAssignedEvent(Guid.NewGuid(), commandId, Revision + 1, nowUtc, principal, assignment.DefensiveCopy()));
    }
        ...
        DateTime nowUtc, string principal)
    {
        ...
        return CommitAlreadyApplied(new FundCompositionReservedEvent(Guid.NewGuid(), commandId, Revision + 1, nowUtc, principal, reservation));
    }
        ...
        Guid commandId, AddManualFundOrderTradeRequest request, DateTime nowUtc, string principal)
    {
        ...
        return CommitAlreadyApplied(new FundManualOrderChangedEvent(
    ...
    IPortfolioFundDomainEvent ChangeComposition(Guid commandId, long expectedRevision, DateTime nowUtc, string principal,
        Func<FundOrderProjectionReadModel> change)
    {
        RequireCurrent(expectedRevision);
        ValidateCommand(commandId, nowUtc, principal);
        var order = change();
        return CommitAlreadyApplied(new FundCompositionStateChangedEvent(Guid.NewGuid(), commandId, Revision + 1, nowUtc, principal, order));
    }

    IPortfolioFundDomainEvent CommitAlreadyApplied(IPortfolioFundDomainEvent domainEvent)
    {
    ...
    IPortfolioFundDomainEvent ChangeManualComposition(
        Guid commandId,
        DateTime nowUtc,
        string principal,
