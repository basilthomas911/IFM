...
Single-selecting a legacy trade SHALL render its actual supported TradeDb blotter in the lower Trade Orders detail region without closing Trade Orders. The embedded blotter SHALL expose the same stored trade details, option-leg information, position/P&L history, futures EOD graph, and spread-distribution graph as the main viewer. Selection replacement SHALL await closure and disposal of the preceding viewer and SHALL fence delayed work by selection generation. The lower detail region SHALL NOT render a legacy-composition surrogate: a missing TradeDb definition SHALL show only `No corresponding TradeDb trade exists for OrderId:TradeId`, and an unsupported type SHALL show only that no real blotter is available. `View Legacy Trade` and legacy-trade double-click SHALL continue returning the accepted historical selection, source Fund, and source FundOrder to the main application. The main application SHALL create or activate one middle-screen tab named `OrderId:TradeId`; repeated opening SHALL not duplicate that legacy tab. Both embedded and tab-hosted supported Iron Condors SHALL use the actual Iron Condor blotter under an immutable historical-read-only flag: live-feed controls are hidden and disabled, no reset/streaming listener is registered, and any attempted command or write entry point fails closed before service execution. Historical value date SHALL prefer the latest stored position date, then TradeDb trade date, then composition trade date. The Trade Orders form SHALL use a compact resizable layout with a 900-pixel default client height and an adaptive detail region.
