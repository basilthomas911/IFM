...
> **Legacy Fund cutover (2026-09-21):** The legacy Fund domain, APIs, storage, UI, import/history path, and financial writer-fence/migration infrastructure have been removed. Canonical Portfolio/Fund actors and Portfolio financial schema version 2 are current. Requirements below that keep legacy services operational, retain legacy UI/history, or prescribe legacy migration/fencing are superseded and remain only as historical design context.
...
The implemented development UI separates setup, explicitly entered opening capital, reconciliation, source qualification and spending-authority review. Qualification SHALL verify the trusted Development host, unqualified Emulator book, matched ledger reconciliation, and exact canonical Portfolio/Fund source versions, then leave CanSpend false until authority is separately reviewed. No historical balance import, automatic capital amount, or production opening capital is authorized.
