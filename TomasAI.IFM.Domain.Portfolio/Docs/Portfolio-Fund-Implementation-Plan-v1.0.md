...
> **Legacy Fund cutover completed (2026-09-21):** The legacy Fund domain, APIs, storage, UI, import/history path, and financial writer-fence/migration infrastructure have been removed. Earlier PF stages and acceptance notes that require legacy services to remain operational are historical evidence, not outstanding or current runtime requirements.
...
> **Actor responsibility revision (2026-09-08):** The financial phase uses exactly two Function actors: `CapacityReservationFunctionActor` and `CapacityConsumptionFunctionActor`. `GeneralLedgerCommandActor` handles single/batch posting; `CapacityReservationCommandActor` handles subsequent lifecycle changes. Configuration uses Command actors and reads use Query actors. Both financial Command and Function writes require atomic PostgreSQL business/receipt/event persistence; changing actor type does not permit eventual financial authority. This supersedes the earlier four-Function proposal. The initial status was Not Started; section 22 records the current development gate status.
...
Current development scope follows the owner's read-only retention and separate development-capital decision. The final sequential qualification and requirement traceability are in [the 2026-09-09 gate evidence report](Portfolio-Financial-Gate-Evidence-2026-09-09.md). Earlier checkpoints below retain their historical status and are superseded by section 22.11.

| Gate | Status at this revision | Completion evidence |
| --- | --- | --- |
| PF-FIN-01 | Complete | Contract/writer/storage manifests, exhaustive legacy-kind disposition and 26 financial wire-verification cases; retained records recognize no capital |
| PF-FIN-02 | Complete | Sixteen authoritative write-failure points, actual lost COMMIT acknowledgements, independent actor/process contention and replay after process exit, durable history/notification recovery and shared lifecycle regressions |
| PF-FIN-03 | Complete | Single/batch posting, database balance/immutability constraints, accounting/corrections/periods, current configuration/query/reconciliation and Scylla history qualification |
| PF-FIN-04 | Complete | Two capacity Functions and lifecycle Command; cross-Fund caps, withdrawal/hold races, consumption/expiry/partial-fill/close conservation and current-authority checks |
| PF-FIN-05 | Complete | All five real calculation stages for 12 variants across three horizons; actual Risk events feed persisted reservation and exact Fund authorization; stable Authorized-intent recovery and labelled execution-fact boundaries. Broker/emulator dispatch remains deferred |
| PF-FIN-06 | Complete | Selected retained-history mode, full canonical-date inventory, writer fence/drain, immutable zero-capital seal and replay; read-only legacy UI, separate development capital and account/rule version editors |
| PF-FIN-07 | Complete | Sequential 1,511-test qualification with no failures/skips, rendered UI, declared load budgets, API build/startup, operational runbook and FIN-T01–18 traceability |
...
### 22.11 Development gate closure — retained history and separate capital — 2026-09-09 UTC

This checkpoint supersedes the remaining-work/status notes in 22.1–22.10. PF-FIN-01 through PF-FIN-07 are Complete for the owner's current development scope. The selected migration delivery retains original legacy data read-only with zero recognized capital; reconstruction of historical journals is outside that selected delivery. Development capital remains separately entered on new development books. Production security and the future broker emulator remain explicitly deferred.

The final sequential runner passed 1,511 tests with zero failures/skips, rendered and checked financial WinForms, built the API with zero warnings/errors and passed Development startup verification. Evidence is `TestResults/portfolio-financial-20260909-135624`; the [checked-in evidence report](Portfolio-Financial-Gate-Evidence-2026-09-09.md) gives suite counts, reproducible commands, FIN-T01–18 traceability and fixture boundaries.

New closure evidence includes 36 real five-stage variant/horizon paths continuing from actual persisted Risk results to reservation and exact fenced Fund authorization; published Risk policy versions; sixteen authoritative write-failure points; two independent OS processes for each financial actor category followed by a fresh-process replay; cross-Fund capacity/withdrawal races; canonical legacy source retention/fencing/drain/seal/replay; and account/rule version editing with durable operation recovery. Existing accounting, lifecycle, source/authorization-scope, history, export and unknown-commit tests remain enabled. The latest load results meet the previously declared budgets and reconcile committed journals/usage.

Application source cutover and entry of an actual capital amount are operational actions, not effects of these tests. Generated test scopes alone were qualified/funded. Historical Funds remain permanent Draft; the Authorized workflow checkpoint is not broker acceptance and does not dispatch the deferred execution owner. The source-retention maintenance procedure and unresolved-write recovery rules remain mandatory when applying the code to an application's records.