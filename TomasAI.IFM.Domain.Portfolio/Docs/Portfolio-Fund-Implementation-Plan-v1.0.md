...
| PF-31 | Complete | 2026-08-31: sequence-allocated Draft Portfolio 1101 `Legacy Test Portfolio` has 14 permanent-Draft, read-only mappings (new FundIds 5001-5014) to every defined legacy Fund. A repeated importer run skipped all mappings idempotently. Production Portfolio NATS queries reconciled all 1,217 FundOrders and 1,222 FundOrderTrades: 1,206/1,204 belong to mapped Funds and 11/18 under orphan source FundIds 1003/1016 remain separately queryable quarantine. TradeDb hydration demonstrated `NoTradeDbDefinition`, `DefinitionOnly`, `PositionHistory`, and `FillHistory`. Current PortfolioDb order loading remains a separate default mode; legacy mode disables every mutation. Single selection now embeds the actual graph-enabled Iron Condor blotter in the lower Trade Orders region while keeping the selector open; selection replacement is disposed and generation-fenced. The legacy-composition surrogate was removed from that region, and missing/unsupported TradeDb cases show only a concise unavailable message. Accepted selections still open one reusable middle-screen `OrderId:TradeId` tab. Qualification proves legacy orders without embedded trade arrays still query by identity, embedded and tab-hosted actual blotters retain both graph controls, zero-size construction is safe, duplicate tabs are prevented, live feed remains disabled, and listener/write entry points fail closed. PF-31 passed its unit, BDD, integration, verification, UI system, live NATS reconciliation, and affected API/UI/importer build suites with zero code/test failures. |
