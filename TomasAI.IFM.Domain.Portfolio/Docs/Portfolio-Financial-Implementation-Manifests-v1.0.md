...
> **Legacy Fund removal (2026-09-21):** Legacy Fund actors, storage, import/history UI, financial inventory/migration stores, and writer fences have been removed. Portfolio financial schema version 2 drops the obsolete PostgreSQL objects. Legacy sections below are retained only as dated implementation history; they are not current runtime contracts or procedures.
...
Status: current development gates complete, 2026-09-09. Current baseline `7a456468` plus the retained-history delivery. See [executed gate evidence](Portfolio-Financial-Gate-Evidence-2026-09-09.md); historical checkpoints below preserve their original scope.
...
The Risk route uses Function, with no legacy Start dispatch. Preparation resolves the exact deployment PipelineParameters RiskManagement reference and verifies its published policy/hash/effectivity. It queries Portfolio through IPortfolioFinancialApi and commits the request before dispatch. Completion recomputes the result and rejects rehashed quantity/side/identity changes. A normal rejection completes NoTrade; an approved proposal remains Started until the exact reservation and Fund authorization establish the persisted Authorized intent and Proceed decision. The workflow does not dispatch consumption or emulator submission. Execution ownership, emulator design/implementation and broker integration remain future work. Current recovery and labelled financial-boundary qualification are recorded in the gate evidence report.
...
- Historical only: schema version 1 used `legacy_financial_inventory`, `legacy_financial_inventory_row`, and `ledger_migration`. Schema version 2 removes them; canonical Portfolio financial state and receipts remain authoritative.
...
- LedgerConfigurationAction appends QualifyDevelopmentBook=11; prior values remain unchanged. It requires trusted Development host policy, an exact nonspending/unqualified Emulator book, matched reconciliation, and current canonical Portfolio source versions. It moves the book to NeedsRefresh without enabling spending.
- Historical only: `legacy_writer_scope`, `legacy_write_intent`, the event-log fence trigger, and the FundDb writer boundary were removed with the legacy Fund subsystem. Canonical Portfolio actors are the only supported financial writers.
- Qualification replay returns the original receipt even after development funding is disabled. No application Fund is funded, migrated, or automatically activated by qualification; integration data belongs to generated test scopes.
...
See [development qualification and runbook](Portfolio-Financial-Development-Qualification-v1.0.md) for measured load and recovery boundaries. Complete five-stage and multi-host qualification remains separate; the legacy import/cutover gate was retired with the legacy Fund subsystem.
