...
/// <summary>Compatibility handler for the published PortfolioQuery route; FinancialPolicy owns new traffic.</summary>
public static class GetActivePortfolioFinancialPolicy
{
    /// <summary>Forwards the legacy query to the FinancialPolicy-owned implementation.</summary>
    ...
    public static ValueTask ExecuteAsync(
        this GetActivePortfolioFinancialPolicyQuery query,
        IQueryActorContext<PortfolioQueryActor> context,
        PortfolioQueryParameters parameters,
        CancellationToken cancellationToken) =>
        TomasAI.IFM.Domain.Portfolio.FinancialPolicy.Query.GetActivePortfolioFinancialPolicy.ExecuteAsync(query, context, parameters, cancellationToken);
}
