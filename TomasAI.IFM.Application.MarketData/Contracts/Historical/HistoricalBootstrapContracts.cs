...
/// <summary>Identifies the durable bootstrap lifecycle.</summary>
public enum HistoricalBootstrapStatus : byte
{
...
public sealed record HistoricalBootstrapGap(DateOnly ValueDate, string SeriesKey, string ReasonCode);

/// <summary>Describes one actual-contract transition in a continuation series.</summary>
public sealed record HistoricalBootstrapRoll(
    string SeriesKey,
    DateOnly ValueDate,
...
public sealed record HistoricalBootstrapAudit(
    int ValidSessionCount,
    IReadOnlyList<HistoricalBootstrapGap> Gaps,
    IReadOnlyList<HistoricalBootstrapRoll> Rolls);

/// <summary>Provides the durable command/query state for one bootstrap attempt.</summary>
public sealed record HistoricalBootstrapState
{
    ...
    public HistoricalBootstrapStatus Status { get; init; }
    ...
    public HistoricalBootstrapAudit? Audit { get; init; }
...
/// <summary>Persists bootstrap checkpoints and immutable manifests in PostgreSQL.</summary>
public interface IHistoricalBootstrapStore
{
    /// <summary>Gets an attempt by its stable identity.</summary>
    ValueTask<HistoricalBootstrapState?> GetAsync(Guid attemptId, CancellationToken cancellationToken);
    /// <summary>Gets an already-completed attempt for a request hash.</summary>
    ValueTask<HistoricalBootstrapState?> GetCompletedByRequestHashAsync(
        string requestSha256,
        CancellationToken cancellationToken);
    /// <summary>Creates or advances an attempt atomically.</summary>
    ValueTask SaveAsync(HistoricalBootstrapState state, CancellationToken cancellationToken);
}
...
/// <summary>Provides a no-op replay target for raw-bootstrap-only processes.</summary>
