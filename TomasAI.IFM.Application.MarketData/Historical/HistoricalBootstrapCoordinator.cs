...
/// </summary>
public sealed class HistoricalBootstrapCoordinator
{
    private readonly IMarketDataHistoricalApi historicalApi;
    private readonly IHistoricalBootstrapStore bootstrapStore;
    ...
    /// <summary>Initializes the bootstrap coordinator.</summary>
    public HistoricalBootstrapCoordinator(
        ...
        TimeProvider timeProvider)
    {
        ...
        this.bootstrapStore = bootstrapStore ?? throw new ArgumentNullException(nameof(bootstrapStore));
    ...
    /// <summary>Executes or resumes one bootstrap and reuses an immutable completed request.</summary>
    public async ValueTask<HistoricalBootstrapState> ExecuteAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (await bootstrapStore.GetCompletedByRequestHashAsync(
                estimate.RequestSha256, cancellationToken).ConfigureAwait(false) is { } completed)
        {
            return completed;
        }

        var existing = await bootstrapStore.GetAsync(
        ...
        var state = new HistoricalBootstrapState
        {
            BootstrapAttemptId = request.BootstrapAttemptId,
        ...
        await bootstrapStore.SaveAsync(state, cancellationToken).ConfigureAwait(false);

        var sink = new BootstrapSink(
            request, observationStore, replayPublisher, calendar, bootstrapStore, state, timeProvider);
        try
        {
            ...
            state = state with
            {
                Status = HistoricalBootstrapStatus.Completed,
    ...
    sealed class BootstrapSink : IHistoricalObservationSink, IHistoricalAcquisitionCheckpointSink
    {
            ...
            CancellationToken cancellationToken)
        {
            if (checkpoint.BootstrapAttemptId != request.BootstrapAttemptId)
                throw new InvalidDataException("Acquisition checkpoint does not belong to the active bootstrap attempt.");
            Checkpoint = checkpoint;
            await bootstrapStore.SaveAsync(initialState with
            {
                Checkpoint = Checkpoint,
                UpdatedAtUtc = timeProvider.GetUtcNow()
        ...
        public async ValueTask AcceptAsync(NormalizedHistoricalBatch batch, CancellationToken cancellationToken)
        {
            ...
            if (batch.BootstrapAttemptId != request.BootstrapAttemptId)
                throw new InvalidDataException("Replay batch does not belong to the active bootstrap attempt.");
        ...
        internal async ValueTask<HistoricalBootstrapAudit> CompleteAsync(CancellationToken cancellationToken)
        {
            ...
            var gaps = new List<HistoricalBootstrapGap>();
            ...
            var rolls = sessions.Keys
                ...
                .SelectMany(group =>
                {
                    ...
                    var values = new List<HistoricalBootstrapRoll>();
