...
| 19 | `TryGetOnTheRunFuturesContract`, `UpdateOnTheRunFuturesContractAsync` | Validated | DataBento definitions when a rollover row is incomplete/due or an operator forces provider revalidation | Startup rollover pointer, persisted v3 futures contracts, atomic rollover-set registry | Singular on-the-run lookup, ES/VX set replacement, valid startup reuse, prior-business-day preparation, due-date persistence and blocking validation | FCR unit, BDD, Scylla, NATS, REST and two-cycle verification gates passed |
