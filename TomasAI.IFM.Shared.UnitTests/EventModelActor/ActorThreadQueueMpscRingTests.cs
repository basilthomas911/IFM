...
public sealed class ActorThreadQueueMpscRingTests
{
    [Fact]
    ...
    [Fact]
    public async Task MultipleProducersAndConsumer_ProcessEveryMessageOnceInProducerOrder()
    {
        const int producerCount = 8;
        const int messagesPerProducer = 1_000;
        const int total = producerCount * messagesPerProducer;
        using var queue = CreateQueue(capacity: 256);
        var received = new bool[total];
        var duplicates = 0;
        var producerLastSequence = Enumerable.Repeat(-1, producerCount).ToArray();
        ...
        var consumer = Task.Run(async () =>
        {
            ...
            await foreach (var actorMessage in queue.ReadAllAsync())
            {
                ...
                message.ProducerSequence.Should().BeGreaterThan(producerLastSequence[message.Producer]);
                producerLastSequence[message.Producer] = message.ProducerSequence;
                if (++count == total)
                    return;
            }
        });
        ...
        var producers = Enumerable.Range(0, producerCount).Select(producer => Task.Run(async () =>
        {
            ...
            {
                await queue.EnqueueAsync(new TestActorMessage(
                    producer * messagesPerProducer + index,
                    producer,
                    index));
            }
        }));
    ...
    [Fact]
    public void Options_RequirePowerOfTwoOnlyForMpscRing()
    {
        var ringOptions = new ActorAdmissionOptions
        {
            MailboxImplementation = ActorMailboxImplementation.MpscRing,
        ...
        ringOptions.Invoking(options => options.Validate()).Should().Throw<InvalidOperationException>()
            .WithMessage("*power of two*MpscRing*");
        channelOptions.Invoking(options => options.Validate()).Should().NotThrow();
    }

    static ActorThreadQueueMpscRing CreateQueue(
        int capacity,
        ActorAdmissionController? controller = null)
    {
        var queue = new ActorThreadQueueMpscRing(
            controller ?? ActorAdmissionController.Disabled,
            capacity);
        queue.SetId(new ActorThreadId(ActorType.Command, "MpscRingTest", "1"));
        queue.Start();
        return queue;
    }
        ...
        int producerSequence = 0) : IActorMessage
    {
        ...
        public ActorSubject Subject { get; } = new(ActorType.Command, "MpscRingTest", "Run", sequence.ToString());
