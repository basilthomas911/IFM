...
| 9 | Validators and simple actor/repository/query paths created avoidable objects or async state machines | Extra allocations and continuations increased Gen0 pressure and code complexity | Reused the immutable option validator, removed no-op overwrite validation, and returned direct `ValueTask` wrappers on non-suspending paths |
