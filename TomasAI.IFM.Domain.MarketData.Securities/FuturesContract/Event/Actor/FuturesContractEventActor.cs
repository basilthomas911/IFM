...
using TomasAI.IFM.Domain.MarketData.Securities.FuturesContract.Event.Extensions;
...
public class FuturesContractEventActor(IEventActorContext<FuturesContractEventActor> actorContext)
    : BaseEventActor<FuturesContractEventActor>(actorContext, actorContext.Logger)
{
    public const string Actor = "FuturesContractEvent";
    readonly ILogger<FuturesContractEventActor> _logger = IsArgumentNull.Set(actorContext.Logger);
    protected IFuturesContractEventContext FuturesContractContext { get; } =
        IsArgumentNull.Set(actorContext as IFuturesContractEventContext, nameof(actorContext))!;
    static readonly IReadOnlyDictionary<Type, Func<IEvent, IFuturesContractEventContext, ILogger, ValueTask<bool>>> _receiveMap = new Dictionary<Type, Func<IEvent, IFuturesContractEventContext, ILogger, ValueTask<bool>>>();
    ...
    protected override IEvent ParseMessage(IEventActorContext<FuturesContractEventActor> context, IActorMessage message)
    ...
    protected override async ValueTask ReceiveAsync(IEventActorContext<FuturesContractEventActor> context, IEvent @event)
    {
        IsArgumentNull.Check(context);
        IsArgumentNull.Check(@event);
        var receiveFunc = ResolveMappedEventHandler(@event, _receiveMap);
        _ = await receiveFunc.Invoke(@event, FuturesContractContext, _logger);
    }
    ...
    protected override async ValueTask OnExceptionAsync(IEventActorContext<FuturesContractEventActor> context, ActorThreadId threadId, IEvent @event, Exception ex)
    {
        try
        {
            IsArgumentNull.Check(context);
